    ...
    * Niesen, J. and Wright, W.M., 2012. Algorithm 919: A Krylov subspace algorithm for evaluating the ``φ``-functions
    ...
    while tau_now < tau_end:
        ...
        while j < m:
            ...
            end_nrm = time() - start_nrm
            global_sync.append(end_nrm)
    ...
    NN = len(global_sync) #note gs has more because I account for V^TV and norm
    avg_matvec = sum(matvec_timings) / N
    avg_gs     = sum(global_sync) / NN
    avg_ortho  = sum(ortho) / N

    stats = (step, reject, krystep, exps, conv, m_ret, avg_matvec, avg_gs, avg_ortho)
