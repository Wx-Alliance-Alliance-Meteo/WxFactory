...
"""
 ---> Full ICWY with norm estimate and lagged normalization
...
"""
def icwy_ne1s(τ_out, A, u, tol = 1e-7, m_init = 10, mmin = 10, mmax = 128, iop = 2, task1 = False):
   ...
   while τ_now < τ_end:
      ...
      if j == 0:
         ...
         #---note first vector is not normalized---


      #keep track of previous nrom estimate to scale at next iteration
      prev_nrm_est = 0.0
      ...
      while j < m:
         ...
         #3. Compute values for ICWY
         #temp is of size (j+1)x2. the second column will be V_j^{T}[v_{j-1} v_j]
         local_sum  = V[0:j+1, :] @ V[j-1:j+1, :].T
         ...
         #4. true norm of previous vector
         nrm = numpy.sqrt(global_sum[j-1,0])

         if j == 1:
            β = nrm
         
         #3. if the previous vector is NOT normalized, then scale for arnoldi
         ...
         if (j > 1):
            #for T matrix
            #this holds the loss of orthogonality
            T[j-1, 0:j-1] = -global_sum[0:j-1, 0]  

         #5. orthogonalize 
         tmp     = T[0:j, 0:j].dot(global_sum[0:j,1])
         V[j,:] -= tmp @ V[0:j,:]

         #6. set values for Hessenberg matrix
         H[0:j, j-1] = tmp
