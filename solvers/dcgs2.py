...
"""
 ---> Full Delayed Classical Gram-Schmidt with Re-ortho
    i.e. DCGS2   
...
Returns:
  ...
  - `stats[6]` - number of times resulting to regular communication
...
"""
def dcgs2(τ_out, A, u, tol = 1e-7, m_init = 10, mmin = 10, mmax = 128, iop = 2, task1 = False):
   ...
   reg_comm = 0 #times used regular communication
   numSteps = len(τ_out)
   ...
   reached_mmax    = False #if reached max size, do not normalize final vec
   ...
   while τ_now < τ_end:
      ...
      while j < m:
         ...
         if rank == 0:
            print("just got next vector in basis, V[{},:] ".format(j))

         #2. compute terms for orthogonalization
         if (j == 1 or prev_normalized):
            local_sum  = V[j-1,:] @ V[j,:].T
            ...
            H[j-1,j-1] = global_sum
            V[j,:]    -= V[j-1,:].dot(H[j-1,j-1])
            ...
            #compute norm of previous vec using pythagoras theorem 
            nrm = math.sqrt(global_sum[j-1,0] - global_sum[0:j-1,0].T.dot(global_sum[0:j-1,0]) )

            if rank == 0:
              print("in else for j = {}, nrm = {}".format(j,nrm))
              print("global_sum = {}".format(global_sum))

            #delayed normalization
            global_sum[j-1,1]    = ( global_sum[j-1,1] - global_sum[0:j-1,0].T.dot(global_sum[0:j-1,1])) / nrm**2
            global_sum[0:j-1,1] /= nrm 

            #orthogonalization

            #second ortho pass of previous vector
            V[j-1,:]     -= global_sum[0:j-1,0] @ V[0:j-1,:]
            V[j-1:j+1,:] /= nrm 

            #first ortho pass of current vector
            V[j,:] -= global_sum[:,1] @ V[0:j, :]

            if rank == 0:
              print("first ortho pass of V[j,:], j = {}".format(j))

            #H matrix
            H[0:j-1,j-2] += global_sum[0:j-1,0]
            H[0:j, j-1]   = global_sum[:,1] - ( H[0:j, 0:j-1].dot(global_sum[0:j-1,0]) ) / nrm
      ...
      #but only until we reach the max m size
      if ( not reached_mmax):

         local_sum  = V[0:m,:] @ V[m,:].T
         global_sum = numpy.empty_like(local_sum)
         mpi4py.MPI.COMM_WORLD.Allreduce([local_sum, mpi4py.MPI.DOUBLE], [global_sum, mpi4py.MPI.DOUBLE])

         finalNrm = numpy.sqrt(global_sum[-1] - numpy.sum(global_sum[0:m]**2))

         V[m,:]     -= global_sum[0:m] @ V[0:m,:]
         V[m,:]     /= finalNrm
         H[m,m-1]    = finalNrm
         H[0:m,m-1] += global_sum[0:m]
      ...
      #---end if for normalizing final vec---
      ...
      if rank == 0:
        print("saving last element, j = {}, nrm = {}".format(j, H[j,j-1]))
      ...
      if happy is True:
         ...
         if rank == 0:
            print("omega = {}, tend = {}, err = {}, tau = {}, tol = {}".format(ω ,τ_end ,err ,τ ,tol))
