"""
Module for running GEF. It's more useful to put it in a separate module from __main__ to be able to pre-load
some other modules and swap them based on the configuration chosen by the user and their availability.
The specific use case is to allow an optional CUDA module, which will be loaded if and only if the user asks
for CUDA and there are devices available on the system. We want to still be able to use GEF on a system where
there is no cupy module or no CUDA device.
"""
...
def create_geometry(param: Configuration, ptopo: Optional[DistributedWorld]) -> Geometry:
   ...
   if param.grid_type == 'cartesian2d':
      return Cartesian2D((param.x0, param.x1), (param.z0, param.z1), param.nb_elements_horizontal,
                         ...
                         param.relief_layer_height, param.array_module)
