...
def get_itf_flux(ptopo, geom, flux_e0_x1, flux_e0_x2, flux_e1_x1, flux_e1_x2, flux_e2_x1, flux_e2_x2):
   datatype = flux_e0_x1.dtype
   num_elements = flux_e0_x1.shape[0]
   ...
   return f0_itf_ew, f0_itf_ns, f1_itf_ew, f1_itf_ns, f2_itf_ew, f2_itf_ns
...
def rhs_sw_fv(Q, geom, mtrx, metric, topo, ptopo,
   ...
   f0_itf_ew, f0_itf_ns, f1_itf_ew, f1_itf_ns, f2_itf_ew, f2_itf_ns = get_itf_flux(
      ptopo, geom, flux_e0_x1, flux_e0_x2, flux_e1_x1, flux_e1_x2, flux_e2_x1, flux_e2_x2)


   lon_w_local = geom.lon_itf_i[:, 0]
   lon_e_local = geom.lon_itf_i[:, -1]
   lon_s_local = geom.lon_itf_j[0, :]
   lon_n_local = geom.lon_itf_j[-1, :]

   lat_w_local = geom.lat_itf_i[:, 0]
   lat_e_local = geom.lat_itf_i[:, -1]
   lat_s_local = geom.lat_itf_j[0, :]
   lat_n_local = geom.lat_itf_j[-1, :]

   lon_n, lon_s, lon_w, lon_e = ptopo.send_recv_neighbors(lon_n_local, lon_s_local, lon_w_local, lon_e_local, 0)
   lat_n, lat_s, lat_w, lat_e = ptopo.send_recv_neighbors(lat_n_local, lat_s_local, lat_w_local, lat_e_local, 0)
   ...
   # df1_dx1[:,  0] = (f1_itf_ew[:, 1]   - flux_e1_x1[:, 0]) * 2.0
   # df1_dx1[:, -1] = (flux_e1_x1[:, -1] - f1_itf_ew[:, -2]) * 2.0
   # df1_dx2[0,  :] = (f1_itf_ns[1, :]   - flux_e1_x2[0, :]) * 2.0
   # df1_dx2[-1, :] = (flux_e1_x2[-1, :] - f1_itf_ns[-2, :]) * 2.0
   ...
   plot_array(f0_itf_ew[:, :], filename='f0_itf_ew_fv.png')
   plot_array(f1_itf_ew[:, :], filename='f1_itf_ew_fv.png')
   # plot_array(f1_itf_ew[:, 1:], filename='f1_itf_e_fv.png')
   plot_array(f1_itf_ns[:, :], filename='f1_itf_ns_fv.png')
   ...
   df1_dx1_ext, df1_dx2_ext = ptopo.xchange_halo_vector(geom, df1_dx1, df1_dx2)
   plot_array(df1_dx1_ext, filename='df1_dx1_full_fv.png')
   plot_array(df1_dx2_ext, filename='df1_dx2_full_fv.png')
