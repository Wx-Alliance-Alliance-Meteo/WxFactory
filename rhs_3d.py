...
def rhs_3d(Q, geom, mtrx, metric, topo, ptopo, nb_sol_pts: int, nb_elements_horiz: int, nb_elements_vert: int, case_number: int, filter_rhs: bool = False):
   ...
   for elem in range(nb_elements_horiz):
      ...
      h_itf_i[0, pos, 0, :] = HH[:, epais] @ mtrx.extrap_west
      h_itf_i[0, pos, 1, :] = HH[:, epais] @ mtrx.extrap_east

      u1_itf_i[0, pos, 0, :] = u1[:, epais] @ mtrx.extrap_west
      u1_itf_i[0, pos, 1, :] = u1[:, epais] @ mtrx.extrap_east

      u2_itf_i[0, pos, 0, :] = u2[:, epais] @ mtrx.extrap_west
      u2_itf_i[0, pos, 1, :] = u2[:, epais] @ mtrx.extrap_east

      # --- Direction x2

      h_itf_j[0, pos, 0, :] = mtrx.extrap_south @ HH[epais, :]
      h_itf_j[0, pos, 1, :] = mtrx.extrap_north @ HH[epais, :]

      u1_itf_j[0, pos, 0, :] = mtrx.extrap_south @ u1[epais, :]
      u1_itf_j[0, pos, 1, :] = mtrx.extrap_north @ u1[epais, :]

      u2_itf_j[0, pos, 0, :] = mtrx.extrap_south @ u2[epais, :]
      u2_itf_j[0, pos, 1, :] = mtrx.extrap_north @ u2[epais, :]
   ...
   for itf in range(nb_interfaces_horiz):
      ...
      h_itf_i[0, elem_L, 1, :] -= topo.hsurf_itf_i[elem_L, :, 1]
      h_itf_i[0, elem_R, 0, :] -= topo.hsurf_itf_i[elem_R, :, 0]

      h_itf_j[0, elem_L, 1, :] -= topo.hsurf_itf_j[elem_L, 1, :]
      h_itf_j[0, elem_R, 0, :] -= topo.hsurf_itf_j[elem_R, 0, :]

      # Direction x1

      if shallow_water_equations:
         eig_L[:] = numpy.abs( u1_itf_i[0, elem_L, 1, :] ) + numpy.sqrt( gravity * h_itf_i[0, elem_L, 1, :] * metric.H_contra_11_itf_i[:, itf] )
         eig_R[:] = numpy.abs( u1_itf_i[0, elem_R, 0, :] ) + numpy.sqrt( gravity * h_itf_i[0, elem_R, 0, :] * metric.H_contra_11_itf_i[:, itf] )
      else:
         eig_L[:] = numpy.abs( u1_itf_i[0, elem_L, 1, :] )
         eig_R[:] = numpy.abs( u1_itf_i[0, elem_R, 0, :] )

      eig[:] = numpy.maximum(eig_L, eig_R)

      # --- Continuity equation

      flux_L[:] = metric.sqrtG_itf_i[:, itf] * h_itf_i[0, elem_L, 1, :] * u1_itf_i[0, elem_L, 1, :]
      flux_R[:] = metric.sqrtG_itf_i[:, itf] * h_itf_i[0, elem_R, 0, :] * u1_itf_i[0, elem_R, 0, :]

      flux_Eq0_itf_i[elem_L, :, 1] = 0.5 * ( flux_L  + flux_R - eig * metric.sqrtG_itf_i[:, itf] *
                                             ( h_itf_i[0, elem_R, 0, :] - h_itf_i[0, elem_L, 1, :] ) )
      flux_Eq0_itf_i[elem_R, :, 0] = flux_Eq0_itf_i[elem_L, :, 1]

      # --- u1 equation

      flux_L[:] = metric.sqrtG_itf_i[:, itf] * ( h_itf_i[0, elem_L, 1, :] * u1_itf_i[0, elem_L, 1, :]**2 \
            + 0.5 * gravity * metric.H_contra_11_itf_i[:, itf] * h_itf_i[0, elem_L, 1, :]**2 )
      flux_R[:] = metric.sqrtG_itf_i[:, itf] * ( h_itf_i[0, elem_R, 0, :] * u1_itf_i[0, elem_R, 0, :]**2 \
            + 0.5 * gravity * metric.H_contra_11_itf_i[:, itf] * h_itf_i[0, elem_R, 0, :]**2 )

      flux_Eq1_itf_i[elem_L, :, 1] = 0.5 * ( flux_L  + flux_R - eig * metric.sqrtG_itf_i[:, itf] \
            * ( h_itf_i[0, elem_R, 0, :] * u1_itf_i[0, elem_R, 0, :] - h_itf_i[0, elem_L, 1, :] * u1_itf_i[0, elem_L, 1, :] ) )
      flux_Eq1_itf_i[elem_R, :, 0] = flux_Eq1_itf_i[elem_L, :, 1]

      # --- u2 equation

      flux_L[:] = metric.sqrtG_itf_i[:, itf] * ( h_itf_i[0, elem_L, 1, :] * u2_itf_i[0, elem_L, 1, :] * u1_itf_i[0, elem_L, 1, :] \
            + 0.5 * gravity * metric.H_contra_21_itf_i[:, itf] * h_itf_i[0, elem_L, 1, :]**2 )
      flux_R[:] = metric.sqrtG_itf_i[:, itf] * ( h_itf_i[0, elem_R, 0, :] * u2_itf_i[0, elem_R, 0, :] * u1_itf_i[0, elem_R, 0, :] \
            + 0.5 * gravity * metric.H_contra_21_itf_i[:, itf] * h_itf_i[0, elem_R, 0, :]**2 )

      flux_Eq2_itf_i[elem_L, :, 1] = 0.5 * ( flux_L  + flux_R - eig * metric.sqrtG_itf_i[:, itf] \
            * ( h_itf_i[0, elem_R, 0, :] * u2_itf_i[0, elem_R, 0, :] - h_itf_i[0, elem_L, 1, :] * u2_itf_i[0, elem_L, 1, :] ) )
      flux_Eq2_itf_i[elem_R, :, 0] = flux_Eq2_itf_i[elem_L, :, 1]

      # Direction x2

      if shallow_water_equations:
         eig_L[:] = numpy.abs( u2_itf_j[0, elem_L, 1, :] ) + numpy.sqrt( gravity * h_itf_j[0, elem_L, 1, :] * metric.H_contra_22_itf_j[itf, :] )
         eig_R[:] = numpy.abs( u2_itf_j[0, elem_R, 0, :] ) + numpy.sqrt( gravity * h_itf_j[0, elem_R, 0, :] * metric.H_contra_22_itf_j[itf, :] )
      else:
         eig_L[:] = numpy.abs( u2_itf_j[0, elem_L, 1, :] )
         eig_R[:] = numpy.abs( u2_itf_j[0, elem_R, 0, :] )

      eig[:] = numpy.maximum(eig_L, eig_R)

      # --- Continuity equation

      flux_L[:] = metric.sqrtG_itf_j[itf, :] * h_itf_j[0, elem_L, 1, :] * u2_itf_j[0, elem_L, 1, :]
      flux_R[:] = metric.sqrtG_itf_j[itf, :] * h_itf_j[0, elem_R, 0, :] * u2_itf_j[0, elem_R, 0, :]

      flux_Eq0_itf_j[elem_L, 1, :] = 0.5 * ( flux_L + flux_R - eig * metric.sqrtG_itf_j[itf, :] *
                                             ( h_itf_j[0, elem_R, 0, :] - h_itf_j[0, elem_L, 1, :] ) )
      flux_Eq0_itf_j[elem_R, 0, :] = flux_Eq0_itf_j[elem_L, 1, :]

      # --- u1 equation

      flux_L[:] = metric.sqrtG_itf_j[itf, :] * ( h_itf_j[0, elem_L, 1, :] * u1_itf_j[0, elem_L, 1, :] * u2_itf_j[0, elem_L, 1, :] \
            + 0.5 * gravity * metric.H_contra_12_itf_j[itf, :] * h_itf_j[0, elem_L, 1, :]**2 )
      flux_R[:] = metric.sqrtG_itf_j[itf, :] * ( h_itf_j[0, elem_R, 0, :] * u1_itf_j[0, elem_R, 0, :] * u2_itf_j[0, elem_R, 0, :] \
            + 0.5 * gravity * metric.H_contra_12_itf_j[itf, :] * h_itf_j[0, elem_R, 0, :]**2 )

      flux_Eq1_itf_j[elem_L, 1, :] = 0.5 * ( flux_L + flux_R - eig * metric.sqrtG_itf_j[itf, :] \
            * ( h_itf_j[0, elem_R, 0, :] * u1_itf_j[0, elem_R, 0, :] - h_itf_j[0, elem_L, 1, :] * u1_itf_j[0, elem_L, 1, :]) )
      flux_Eq1_itf_j[elem_R, 0, :] = flux_Eq1_itf_j[elem_L, 1, :]

      # --- u2 equation

      flux_L[:] = metric.sqrtG_itf_j[itf, :] * ( h_itf_j[0, elem_L, 1, :] * u2_itf_j[0, elem_L, 1, :]**2 \
            + 0.5 * gravity * metric.H_contra_22_itf_j[itf, :] * h_itf_j[0, elem_L, 1, :]**2 )
      flux_R[:] = metric.sqrtG_itf_j[itf, :] * ( h_itf_j[0, elem_R, 0, :] * u2_itf_j[0, elem_R, 0, :]**2 \
            + 0.5 * gravity * metric.H_contra_22_itf_j[itf, :] * h_itf_j[0, elem_R, 0, :]**2 )

      flux_Eq2_itf_j[elem_L, 1, :] = 0.5 * ( flux_L + flux_R - eig * metric.sqrtG_itf_j[itf, :] \
            * ( h_itf_j[0, elem_R, 0, :] * u2_itf_j[0, elem_R, 0, :] - h_itf_j[0, elem_L, 1, :] * u2_itf_j[0, elem_L, 1, :]) )
