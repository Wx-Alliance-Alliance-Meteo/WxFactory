...
def output_init(geom, param):
   ...
   if netcdf_serial:
      ...
      lons  = MPI.COMM_WORLD.gather(geom.lon * 180/math.pi, root=0)
      lats  = MPI.COMM_WORLD.gather(geom.lat * 180/math.pi, root=0)
      if param.equations == "euler":
         elevs = MPI.COMM_WORLD.gather(geom.coordVec_latlon[2,:,:,:], root=0)
         topos = MPI.COMM_WORLD.gather(geom.zbot[:,:], root=0)
...
def output_netcdf(Q, geom, metric, mtrx, topo, step, param):
   ...
   if param.equations == "shallow_water":
      ...
      if param.case_number >= 2: # Shallow water
         ...
         store_field(u, 'U', idx, ncfile)
         store_field(v, 'V', idx, ncfile)
         store_field(rv, 'RV', idx, ncfile)
         store_field(pv, 'PV', idx, ncfile)
      ...
      store_field(rho, 'rho', idx, ncfile)
      store_field(u, 'U', idx, ncfile)
      store_field(v, 'V', idx, ncfile)
      store_field(w, 'W', idx, ncfile)
      store_field(theta, 'theta', idx, ncfile)
      store_field(p0 * (Q[idx_rho_theta] * Rd / p0)**(cpd / cvd), 'P', idx, ncfile)

      if param.case_number == 11 or param.case_number == 12:
         store_field(Q[5, :, :, :] / rho, 'q1', idx, ncfile)

      if param.case_number == 11:
         for i in [6, 7, 8]:
            store_field(Q[i, :, :, :] / rho, f'q{i-4}', idx, ncfile)
