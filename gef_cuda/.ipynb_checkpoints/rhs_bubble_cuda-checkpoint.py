...
from common.definitions         import idx_2d_rho, idx_2d_rho_u, idx_2d_rho_w, idx_2d_rho_theta, \
                                       cpd, cvd, gravity, heat_capacity_ratio, p0, Rd
...
class RHSBubble(metaclass=CudaModule,
   ...
   def set_kfaces_var(kfaces_var: NDArray[cp.float64], Q: NDArray[cp.float64], extrap_up: NDArray[cp.float64],
                      extrap_down: NDArray[cp.float64], nbsolpts: int, nb_elements_z: int, nb_elements_x: int): ...

   @cuda_kernel(DimSpec.groupby_first_x(lambda s: Dim(s[2], s[1], s[0])))
   def set_ifaces_var(ifaces_var: NDArray[cp.float64], Q: NDArray[cp.float64], extrap_west: NDArray[cp.float64],
                      extrap_east: NDArray[cp.float64], nbsolpts: int, nb_elements_z: int, nb_elements_x: int): ...

   @cuda_kernel(DimSpec.groupby_first_x(lambda s: Dim(s[3], s[1] - 1, 1)))
   def set_kfaces_flux(kfaces_flux: NDArray[cp.float64], kfaces_pres: NDArray[cp.float64],
                       kfaces_var: NDArray[cp.float64], nb_equations: int, nbsolpts: int, nb_elements_z: int,
                       nb_elements_x: int): ...

   @cuda_kernel(DimSpec.groupby_first_x(lambda s: Dim(s[2], s[1] - 1, 1)))
   def set_ifaces_flux(ifaces_flux: NDArray[cp.float64], ifaces_pres: NDArray[cp.float64],
                       ifaces_var: NDArray[cp.float64], nb_equations: int, nbsolpts: int, nb_elements_z: int,
                       nb_elements_x: int, xperiodic: bool): ...
...
def rhs_bubble_cuda(Q: NDArray[cp.float64], geom: Cartesian2D, mtrx: DFROperators,
   ...
   RHSBubble.set_ifaces_flux(ifaces_flux, ifaces_pres, ifaces_var, nb_equations, nbsolpts, \
                             nb_elements_z, nb_elements_x, geom.xperiodic)
