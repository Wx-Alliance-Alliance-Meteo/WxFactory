...
class Epi_others(Integrator):
   def __init__(self, order: int, rhs: Callable, jac: Callable, exp_solver, world_data, coeff=None, init_method=None, init_substeps: int = 1):
   ...
   def __step__(self, Q: numpy.ndarray, dt: float):
      ...
      for i in range(self.n_prev):
         ...
         J_deltaQ = self.jac(self.previous_Q[i] - Q, Q, 1.0, self.coeff[0], self.coeff[1], self.coeff[2], self.worldInfo)
