...
class Srerk(Integrator):
   ...
   def __step__(self, Q: numpy.ndarray, dt: float):
      ...
      #----- dcgs2 -----
         ...
         z, stats = dcgs2(self.c[0], matvec_handle, vec, tol=self.tol ,m_init=self.krylov_size, mmin=16,  mmax=64, task1=False)
         self.krylov_size = math.floor(0.7 * stats[5] + 0.3 * self.krylov_size)
      ...
      #----- kiops + norm estimate-----
         ...
         z, stats = kiops_nest(self.c[0], matvec_handle, vec, tol=self.tol ,m_init=self.krylov_size, mmin=16,  mmax=64, task1=False)
         self.krylov_size = math.floor(0.7 * stats[5] + 0.3 * self.krylov_size)
      ...
      for i_proj in range(1, self.n_proj):
         ...
         #----- dcgs2 -----
            ...
            z, stats = dcgs2(self.c[i_proj], matvec_handle, vec, tol=self.tol ,m_init=self.krylov_size, mmin=16,  mmax=64, task1=False)
            self.krylov_size = math.floor(0.7 * stats[5] + 0.3 * self.krylov_size)
         ...
         #----- kiops + norm estimate-----
            ...
            z, stats = kiops_nest(self.c[i_proj], matvec_handle, vec, tol=self.tol ,m_init=self.krylov_size, mmin=16,  mmax=64, task1=False)
            self.krylov_size = math.floor(0.7 * stats[5] + 0.3 * self.krylov_size)
