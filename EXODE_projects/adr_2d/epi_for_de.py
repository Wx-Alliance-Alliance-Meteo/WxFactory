...
class Epi_for_DE:
    def __init__(
        ...
        if init_method or self.n_prev == 0:
            ...
            self.init_method = Epi_for_DE(2, rhs, tol, rtol, exponential_solver, exode_method, exode_controller)
    ...
    def step(self, Q: numpy.ndarray, dt: float, first_step: float):
        ...
        if self.exponential_solver == "exode":
            # print(vec.shape)
            # MATRIX = matvec_handle(vec)
            # print(MATRIX)
            # MATRIX.tofile('A_epi'+str(order)+'.csv', sep = ',')
            phiv, stats = exode(
                first_step,
                matvec_handle,
                vec,
        ...
        # print(stats[0], ",", stats[1], ",", stats[2])  # iteration, rejected, tolerance
        ...
        # f' {stats[1]} rejected expm) to a solution with local error {stats[4]:.2e}')
        # print(stats[2], ",", stats[1], ",", f'{stats[4]:.2e}') # iteration, rejected, tolerance
