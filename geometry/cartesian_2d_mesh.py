...
class Cartesian2D(Geometry):
    def __init__(
        ...
        if nb_elements_relief_layer > 0:
            ...
            itf_x3 = xp.linspace(
                start=relief_layer_height, stop=domain_z[1], num=(nb_elements_z - nb_elements_relief_layer) + 1
            )
            itf_relief_layer = xp.linspace(
                start=domain_z[0], stop=relief_layer_height, num=nb_elements_relief_layer + 1
            )

            z1 = xp.zeros(nb_elements_relief_layer * len(self.solutionPoints))
            z2 = xp.zeros((nb_elements_z - nb_elements_relief_layer) * len(self.solutionPoints))
            ...
            x3 = xp.concatenate((z1, z2), axis=None)
    ...
    def make_mountain(self, mountain_type="sine"):
        ...
        if mountain_type == "sine":
            h0 = 250.0  # Max mountain height
            a = 5000.0  # Overall mountain width factor
            lmbda = 4000.0  # Mountain ripple width factor

            def h_func(x):
                return h0 * xp.exp(-((x / a) ** 2)) * xp.cos(xp.pi * x / lmbda) ** 2 + 100.0
            ...
            def h_func(x):
                return xp.where(x > cliff_pos, mountain_height, 0.0)
        ...
        top_boundary = xp.zeros_like(relief_mask)
        left_boundary = xp.zeros_like(relief_mask)
        right_boundary = xp.zeros_like(relief_mask)

        # Top of the mountain: points that are inside, with a neighbor above that is outside
        top_boundary[-1, :] = relief_mask[-1, :]
        top_boundary[:-1, :] = xp.where(xp.diff(relief_mask, axis=0), True, False)

        # Sides of the mountain: points that are inside, with a left/right neighbor that is outside
        # *excluding* top points
        h_diff = xp.diff(relief_mask.astype(int), axis=1)
        left_boundary[:, 1:] = (h_diff == 1) & xp.logical_not(top_boundary[:, 1:])
        right_boundary[:, :-1] = (h_diff == -1) & xp.logical_not(top_boundary[:, :-1])

        if xp.any(left_boundary & right_boundary):
            raise ValueError(
                f"We can't deal with mountains that are too thin, we need to have at least 2 grid points wide (except at the top)"
            )
        ...
        nsq_plus1 = xp.sqrt(ncompx**2 + 1)
        normals_x = ncompx / nsq_plus1
        normals_z = -1.0 / nsq_plus1

        normals_x = xp.where(relief_boundary_mask, normals_x, 0.0)
        normals_z = xp.where(relief_boundary_mask, normals_z, 0.0)

        # print(f'normals_z: {normals_z[:5, :30]}')

        if mountain_type == "step":
            normals_x = xp.select([left_boundary, right_boundary], [1.0, -1.0], normals_x)
            normals_z = xp.where(side_boundary_mask, 0.0, normals_z)

        print_mountain(
            self.X1[:end, :],
            self.X3[:end, :],
            relief_mask + relief_boundary_mask * 2,
            normals_x=normals_x,
            normals_z=normals_z,
        ...
        print(
            f"Number of Terrain points: {xp.count_nonzero(relief_mask)}, "
            f"Number of boundary points: {xp.count_nonzero(relief_boundary_mask)} "
            f"({xp.count_nonzero(left_boundary)} left, {xp.count_nonzero(right_boundary)} right)"
        )
