...
class Metric3DTopo:
    ...
    def build_metric(self):
        ...
        height_ext_i = numpy.stack(
            (height_itf_i[:, :, :-1], height_itf_i[:, :, 1:]), axis=-1  # west boundaries  # east boundaries
        )
        # ext_j shape: (nk, nb_elements_x2, 2, ni)
        height_ext_j = numpy.stack(
            (height_itf_j[:, :-1, :], height_itf_j[:, 1:, :]), axis=-2  # south boundaries  # north boundaries
        )
        # ext_k shape: (nb_elements_x3, 2, nj, ni)
        height_ext_k = numpy.stack(
            (height_itf_k[:-1, :, :], height_itf_k[1:, :, :]), axis=-3  # bottom boundaries  # top boundaries
        )
        ...
        for Hcov, Hcontra, rootG, X, Y, R, dRdx1, dRdx2, dRdeta in zip(
            (H_cov, H_cov_itf_i, H_cov_itf_j, H_cov_itf_k),
            ...
            if deep:
                ...
                rootG[:] = (
                    (Δx / 2) * (Δy / 2) * (Δeta / 2) * R**2 * (1 + X**2) * (1 + Y**2) * xp.abs(dRdeta) / delsq ** (1.5)
                )
                ...
                rootG[:] = (
                    (Δx / 2) * (Δy / 2) * (Δeta / 2) * A**2 * (1 + X**2) * (1 + Y**2) * xp.abs(dRdeta) / delsq ** (1.5)
                )
        ...
        dRdx1_ext_i = xp.stack(
            (dRdx1_itf_i[:, :, :-1], dRdx1_itf_i[:, :, 1:]), axis=-1  # min-i boundary  # max-i boundary
        )
        dRdx1_ext_j = xp.stack(
            (dRdx1_itf_j[:, :-1, :], dRdx1_itf_j[:, 1:, :]), axis=-2  # min-j boundary  # max-j boundary
        )
        dRdx1_ext_k = xp.stack(
            (dRdx1_itf_k[:-1, :, :], dRdx1_itf_k[1:, :, :]), axis=-3  # min-k boundary  # max-k boundary
        )

        dRdx2_ext_j = xp.stack(
            (dRdx2_itf_j[:, :-1, :], dRdx2_itf_j[:, 1:, :]), axis=-2  # min-j boundary  # max-j boundary
        )
        dRdx2_ext_k = xp.stack(
            (dRdx2_itf_k[:-1, :, :], dRdx2_itf_k[1:, :, :]), axis=-3  # min-k boundary  # max-k boundary
        )

        dRdeta_ext_k = xp.stack(
            (dRdeta_itf_k[:-1, :, :], dRdeta_itf_k[1:, :, :]), axis=-3  # min-k boundary  # max-k boundary
        )
        ...
        self.coriolis_f = (
            ...
            * (
                math.sin(geom.lat_p)
                - geom.X * math.cos(geom.lat_p) * math.sin(geom.angle_p)
                + geom.Y * math.cos(geom.lat_p) * math.cos(geom.angle_p)
            )
        )
...
class Metric:
    ...
    def __init__(self, geom: CubedSphere):
        ...
        self.sqrtG_itf_i = (
            ...
            * (1.0 + geom.X_itf_i**2)
            * (1.0 + geom.Y_itf_i**2)
            / (geom.delta2_itf_i * geom.delta_itf_i)
        )
        self.sqrtG_itf_j = (
            geom.earth_radius**2
            * (1.0 + geom.X_itf_j**2)
            * (1.0 + geom.Y_itf_j**2)
            / (geom.delta2_itf_j * geom.delta_itf_j)
        )
        ...
        self.sqrtG *= geom.Δx1 * geom.Δx2 * geom.Δx3 / 8.0
        self.sqrtG_itf_i *= geom.Δx1 * geom.Δx2 * geom.Δx3 / 8.0
        self.sqrtG_itf_j *= geom.Δx1 * geom.Δx2 * geom.Δx3 / 8.0
        ...
        self.H_cov_11 *= geom.Δx1**2 / 4.0
        self.H_cov_12 *= geom.Δx1 * geom.Δx2 / 4.0
        self.H_cov_13 *= geom.Δx1 * geom.Δx3 / 4.0
        self.H_cov_21 *= geom.Δx2 * geom.Δx1 / 4.0
        self.H_cov_22 *= geom.Δx2**2 / 4.0
        self.H_cov_23 *= geom.Δx2 * geom.Δx3 / 4.0
        self.H_cov_33 *= geom.Δx3**2 / 4.0

        self.H_contra_11 *= 4.0 / (geom.Δx1**2)
        self.H_contra_12 *= 4.0 / (geom.Δx1 * geom.Δx2)
        self.H_contra_13 *= 4.0 / (geom.Δx1 * geom.Δx3)

        self.H_contra_21 *= 4.0 / (geom.Δx1 * geom.Δx2)
        self.H_contra_22 *= 4.0 / (geom.Δx2**2)
        self.H_contra_23 *= 4.0 / (geom.Δx3 * geom.Δx2)

        self.H_contra_31 *= 4.0 / (geom.Δx1 * geom.Δx3)
        self.H_contra_32 *= 4.0 / (geom.Δx2 * geom.Δx3)
        self.H_contra_33 *= 4.0 / (geom.Δx3**2)

        self.H_contra_11_itf_i *= 4.0 / (geom.Δx1**2)
        self.H_contra_12_itf_i *= 4.0 / (geom.Δx1 * geom.Δx2)
        self.H_contra_13_itf_i *= 4.0 / (geom.Δx1 * geom.Δx3)

        self.H_contra_21_itf_i *= 4.0 / (geom.Δx1 * geom.Δx2)
        self.H_contra_22_itf_i *= 4.0 / (geom.Δx2**2)
        self.H_contra_23_itf_i *= 4.0 / (geom.Δx2 * geom.Δx3)

        self.H_contra_31_itf_i *= 4.0 / (geom.Δx1 * geom.Δx3)
        self.H_contra_32_itf_i *= 4.0 / (geom.Δx2 * geom.Δx3)
        self.H_contra_33_itf_i *= 4.0 / (geom.Δx3**2)

        self.H_contra_11_itf_j *= 4.0 / (geom.Δx1**2)
        self.H_contra_12_itf_j *= 4.0 / (geom.Δx1 * geom.Δx2)
        self.H_contra_13_itf_j *= 4.0 / (geom.Δx1 * geom.Δx3)

        self.H_contra_21_itf_j *= 4.0 / (geom.Δx1 * geom.Δx2)
        self.H_contra_22_itf_j *= 4.0 / (geom.Δx2**2)
        self.H_contra_23_itf_j *= 4.0 / (geom.Δx2 * geom.Δx3)

        self.H_contra_31_itf_j *= 4.0 / (geom.Δx1 * geom.Δx3)
        self.H_contra_32_itf_j *= 4.0 / (geom.Δx2 * geom.Δx3)
        self.H_contra_33_itf_j *= 4.0 / (geom.Δx3**2)
