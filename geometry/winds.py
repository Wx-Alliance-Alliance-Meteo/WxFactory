...
def wind2contra_2d(u: Union[float, numpy.ndarray], v: Union[float, numpy.ndarray], geom: CubedSphere):
    ...
    denom = numpy.sqrt(
        (
            math.cos(geom.lat_p)
            + geom.X * math.sin(geom.lat_p) * math.sin(geom.angle_p)
            - geom.Y * math.sin(geom.lat_p) * math.cos(geom.angle_p)
        )
        ** 2
        + (geom.X * math.cos(geom.angle_p) + geom.Y * math.sin(geom.angle_p)) ** 2
    )

    dx1dlon = math.cos(geom.lat_p) * math.cos(geom.angle_p) + (
        geom.X * geom.Y * math.cos(geom.lat_p) * math.sin(geom.angle_p) - geom.Y * math.sin(geom.lat_p)
    ) / (1.0 + geom.X**2)
    dx2dlon = (geom.X * geom.Y * math.cos(geom.lat_p) * math.cos(geom.angle_p) + geom.X * math.sin(geom.lat_p)) / (
        1.0 + geom.Y**2
    ) + math.cos(geom.lat_p) * math.sin(geom.angle_p)

    dx1dlat = (
        -geom.delta2
        * ((math.cos(geom.lat_p) * math.sin(geom.angle_p) + geom.X * math.sin(geom.lat_p)) / (1.0 + geom.X**2))
        / denom
    )
    dx2dlat = (
        geom.delta2
        * ((math.cos(geom.lat_p) * math.cos(geom.angle_p) - geom.Y * math.sin(geom.lat_p)) / (1.0 + geom.Y**2))
        / denom
    )

    # transform to the reference element

    u1_contra = (dx1dlon * lambda_dot + dx1dlat * phi_dot) * 2.0 / geom.Δx1
    u2_contra = (dx2dlon * lambda_dot + dx2dlat * phi_dot) * 2.0 / geom.Δx2

    return u1_contra, u2_contra


def wind2contra_3d(
    u: Union[float, numpy.ndarray],
...
def contra2wind_2d(u1: Union[float, numpy.ndarray], u2: Union[float, numpy.ndarray], geom: CubedSphere):
    ...
    u1_contra = u1 * geom.Δx1 / 2.0
    u2_contra = u2 * geom.Δx2 / 2.0

    denom = (
        math.cos(geom.lat_p)
        + geom.X * math.sin(geom.lat_p) * math.sin(geom.angle_p)
        - geom.Y * math.sin(geom.lat_p) * math.cos(geom.angle_p)
    ) ** 2 + (geom.X * math.cos(geom.angle_p) + geom.Y * math.sin(geom.angle_p)) ** 2

    dlondx1 = (
        (math.cos(geom.lat_p) * math.cos(geom.angle_p) - geom.Y * math.sin(geom.lat_p)) * (1.0 + geom.X**2) / denom
    )

    dlondx2 = (
        (math.cos(geom.lat_p) * math.sin(geom.angle_p) + geom.X * math.sin(geom.lat_p)) * (1.0 + geom.Y**2) / denom
    )
    ...
    dlatdx1 = -(
        (
            geom.X * geom.Y * math.cos(geom.lat_p) * math.cos(geom.angle_p)
            + geom.X * math.sin(geom.lat_p)
            + (1.0 + geom.Y**2) * math.cos(geom.lat_p) * math.sin(geom.angle_p)
        )
        * (1.0 + geom.X**2)
    ) / (geom.delta2 * denom)
    ...
    dlatdx2 = (
        (
            (1.0 + geom.X**2) * math.cos(geom.lat_p) * math.cos(geom.angle_p)
            + geom.X * geom.Y * math.cos(geom.lat_p) * math.sin(geom.angle_p)
            - geom.Y * math.sin(geom.lat_p)
        )
        * (1.0 + geom.Y**2)
    ) / (geom.delta2 * denom)
