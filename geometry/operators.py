...
class DFROperators:
    ...
    def __init__(self, grd: Geometry, param: Configuration):
        ...
        extrap_neg = (legvander(numpy.array([-1.0], like=V), grd.nbsolpts - 1) @ invV).reshape((-1,))
        extrap_pos = (legvander(numpy.array([+1.0], like=V), grd.nbsolpts - 1) @ invV).reshape((-1,))
        ...
        if param.filter_apply:
            ...
            self.diff_ext[numpy.abs(self.diff_ext) < 1e-20] = 0.0
    ...
    def comma_i(
        self: Self, field_interior: NDArray[T], border_i: NDArray[T], grid: CubedSphere, out: NDArray[T] | None = None
    ) -> NDArray[T]:
    ...
    def extrapolate_i(
        self: Self, field_interior: NDArray[T], grid: CubedSphere, out: NDArray[T] | None = None
    ) -> NDArray[T]:
    ...
    def comma_j(
        self: Self, field_interior: NDArray[T], border_j: NDArray[T], grid: CubedSphere, out: NDArray[T] | None = None
    ) -> NDArray[T]:
    ...
    def comma_k(
        self: Self, field_interior: NDArray[T], border_k: NDArray[T], grid: CubedSphere, out: NDArray[T] | None = None
    ) -> NDArray[T]:
