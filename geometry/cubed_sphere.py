...
class CubedSphere(Geometry):
    def __init__(
        ...
        if ztop > 0:  # Note that x3 and eta are 3D arrays
            ...
            x3[:] = (
                domain_x3[0]
                + Δx3 * elements_x3[:, numpy.newaxis, numpy.newaxis, numpy.newaxis]
            ...
            eta[:] = (
                domain_eta[0]
                + Δeta * elements_x3[:, numpy.newaxis, numpy.newaxis, numpy.newaxis]
        ...
        coordVec_num = numpy.stack(
            (numpy.broadcast_to(x1[None, None, :], eta.shape), numpy.broadcast_to(x2[None, :, None], eta.shape), eta)
        )
        ...
        coordVec_num_itf_i = numpy.stack(
            (
                numpy.broadcast_to(x1_itf_i[None, None, :], eta_itf_i.shape),
                numpy.broadcast_to(x2_itf_i[None, :, None], eta_itf_i.shape),
                eta_itf_i,
            )
        )
        coordVec_num_itf_j = numpy.stack(
            (
                numpy.broadcast_to(x1_itf_j[None, None, :], eta_itf_j.shape),
                numpy.broadcast_to(x2_itf_j[None, :, None], eta_itf_j.shape),
                eta_itf_j,
            )
        )
        coordVec_num_itf_k = numpy.stack(
            (
                numpy.broadcast_to(x1_itf_k[None, None, :], eta_itf_k.shape),
    ...
    def apply_topography(self, zbot, zbot_itf_i, zbot_itf_j):
        ...
        self.x3_itf_i[:, :, :] = (
            zbot_itf_i[numpy.newaxis, :, :] + (ztop - zbot_itf_i[numpy.newaxis, :, :]) * self.eta_itf_i
        )  # shape k, j, itf_i
        self.x3_itf_j[:, :, :] = (
            zbot_itf_j[numpy.newaxis, :, :] + (ztop - zbot_itf_j[numpy.newaxis, :, :]) * self.eta_itf_j
        )  # shape k, itf_j, i
        self.x3_itf_k[:, :, :] = (
            zbot[numpy.newaxis, :, :] + (ztop - zbot[numpy.newaxis, :, :]) * self.eta_itf_k
        )  # shape itf_k, j, i
    ...
    def _build_physical_coordinates(self):
        ...
        for coordgnom, coordnum, z in zip(
        ...
        for coord_cart, coord_gnom in zip(
            ...
            coord_cart[0, :] = (
                (self.earth_radius + coord_gnom[2, :])
                / delt
                * (
        ...
        for latlon, cart, gnom in zip(
            [coordVec_latlon, coordVec_latlon_itf_i, coordVec_latlon_itf_j, coordVec_latlon_itf_k],
            [coordVec_cart, coordVec_cart_itf_i, coordVec_cart_itf_j, coordVec_cart_itf_k],
            [coordVec_gnom, coordVec_gnom_itf_i, coordVec_gnom_itf_j, coordVec_gnom_itf_k],
        ):
