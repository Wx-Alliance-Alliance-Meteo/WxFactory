...
def image_field(geom: 'Cartesian2D', field: numpy.ndarray, filename: str, vmin: float, vmax: float, n: int, \
   ...
   if not geom.xperiodic:
      X1_np = geom.X1.get() if isinstance(geom.X1, cp.ndarray) else geom.X1
      X3_np = geom.X3.get() if isinstance(geom.X3, cp.ndarray) else geom.X3
      field_np = field.get() if isinstance(field, cp.ndarray) else field
      cmap = matplotlib.pyplot.contourf(X1_np, X3_np, field_np, cmap=colormap,
