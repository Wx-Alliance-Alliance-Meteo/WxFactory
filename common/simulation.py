      ...
      if self.config.grid_type == 'cubed_sphere':
         ...
         if MPI.COMM_WORLD.rank == 0:
            if self.config.nb_elements_horizontal_total != self.config.nb_elements_horizontal:
               print(f'Adjusting horizontal number of elements from {self.config.nb_elements_horizontal_total} (total) '
   ...
   def _create_geometry(self) -> Geometry:
      ...
      if self.config.grid_type == 'cubed_sphere' and self.process_topo is not None:
         if self.config.equations == 'shallow_water':
            return CubedSphere2D(self.config.nb_elements_horizontal, self.config.nb_elements_vertical,
   ...
   def _determine_starting_state(self):
      ...
      if starting_step > 0:
         try:
            ...
            if self.rank == 0:
               print(f'WARNING: Tried to start from timestep {starting_step}, but unable to read initial state'
