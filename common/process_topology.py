...
class ProcessTopology:
    ...
    def __init__(self, device: Device, rank: Optional[int] = None, comm: MPI.Comm = MPI.COMM_WORLD):
        ...
        if self.my_row == self.nb_lines_per_panel - 1:
            if self.my_panel == 0:
                ...
                my_north = rank_from_location(
                    neighbor_panels[NORTH], self.nb_lines_per_panel - 1, self.nb_lines_per_panel - 1 - self.my_col
                )
                self.convert_contra[NORTH] = lambda a1, a2, coord: (-a1 + 2.0 * coord / (1.0 + coord**2) * a2, -a2)
        ...
        if self.my_row == 0:
            if self.my_panel == 0:
                ...
                my_south = rank_from_location(
                    neighbor_panels[SOUTH], self.nb_elems_per_line - 1 - self.my_col, self.nb_lines_per_panel - 1
                )
        ...
        if self.my_col == 0:
            if self.my_panel == 4:
                my_west = rank_from_location(
                    neighbor_panels[WEST], self.nb_lines_per_panel - 1, self.nb_lines_per_panel - 1 - self.my_row
                )
                self.flip[WEST] = True
                self.convert_contra[WEST] = lambda a1, a2, coord: (-2.0 * coord / (1.0 + coord**2) * a1 - a2, a1)
            elif self.my_panel == 5:
                ...
                self.convert_contra[WEST] = lambda a1, a2, coord: (a1, 2.0 * coord / (1.0 + coord**2) * a1 + a2)
        else:
        ...
        if self.my_col == self.nb_elems_per_line - 1:
            if self.my_panel == 4:
                ...
                self.convert_contra[EAST] = lambda a1, a2, coord: (-2.0 * coord / (1.0 + coord**2) * a1 + a2, -a1)
            elif self.my_panel == 5:
                ...
                self.convert_contra[EAST] = lambda a1, a2, coord: (a1, -2.0 * coord / (1.0 + coord**2) * a1 + a2)
        else:
    ...
    def xchange_scalars(self, geom, field_itf_i, field_itf_j, blocking=True):
        ...
        request, n_recv_buf, s_recv_buf, w_recv_buf, e_recv_buf = self.send_recv_neighbors(
            s_send, n_send, w_send, e_send, flip_dim, sync=False
        )
