...
class ProcessTopology:
    def __init__(self, device: Device):
        ...
        if self.my_row == self.nb_lines_per_panel - 1:
            if self.my_panel == 0:
                ...
                my_north = rank_from_location(
                    my_north_panel, self.nb_lines_per_panel - 1, self.nb_lines_per_panel - 1 - self.my_col
                )
                self.convert_contra_north = lambda a1, a2, X: (-a1 + 2.0 * X / (1.0 + X**2) * a2, -a2)
        ...
        if self.my_row == 0:
            if self.my_panel == 0:
                ...
                my_south = rank_from_location(
                    my_south_panel, self.nb_elems_per_line - 1 - self.my_col, self.nb_lines_per_panel - 1
                )
        ...
        if self.my_col == 0:
            if self.my_panel == 4:
                my_west = rank_from_location(
                    my_west_panel, self.nb_lines_per_panel - 1, self.nb_lines_per_panel - 1 - self.my_row
                )
                self.flip_west = True
                self.convert_contra_west = lambda a1, a2, Y: (-2.0 * Y / (1.0 + Y**2) * a1 - a2, a1)
            elif self.my_panel == 5:
                ...
                self.convert_contra_west = lambda a1, a2, Y: (a1, 2.0 * Y / (1.0 + Y**2) * a1 + a2)
        else:
        ...
        if self.my_col == self.nb_elems_per_line - 1:
            if self.my_panel == 4:
                ...
                self.convert_contra_east = lambda a1, a2, Y: (-2.0 * Y / (1.0 + Y**2) * a1 + a2, -a1)
            elif self.my_panel == 5:
                ...
                self.convert_contra_east = lambda a1, a2, Y: (a1, -2.0 * Y / (1.0 + Y**2) * a1 + a2)
        else:
    ...
    def send_recv_neighbors(
        ...
        for do_flip, data, buffer in zip(
    ...
    def xchange_scalars(self, geom, field_itf_i, field_itf_j, blocking=True):
        ...
        request, n_recv_buf, s_recv_buf, w_recv_buf, e_recv_buf = self.send_recv_neighbors(
            n_send, s_send, w_send, e_send, flip_dim, sync=False
        )
        request = ScalarNonBlockingExchangeRequest(
            (n_recv_buf, s_recv_buf, w_recv_buf, e_recv_buf), (n_recv, s_recv, w_recv, e_recv), request
        )
    ...
    def xchange_sw_interfaces(self, geom, h_i, h_j, u1_i, u2_i, u1_j, u2_j, blocking=True):
        ...
        for do_flip, convert, positions, h, (u1, u2), buffer in zip(
            ...
            [X, X, Y, Y],
            [h_n, h_s, h_w, h_e],
            [(u1_n, u2_n), (u1_s, u2_s), (u1_w, u2_w), (u1_e, u2_e)],
            [send_buffer[0], send_buffer[1], send_buffer[2], send_buffer[3]],
        ):
        ...
        request = ShallowWaterExchangeRequest(
            receive_buffer,
            (
                (h_n_dest, u1_n_dest, u2_n_dest),
                (h_s_dest, u1_s_dest, u2_s_dest),
                (h_w_dest, u1_w_dest, u2_w_dest),
                (h_e_dest, u1_e_dest, u2_e_dest),
            ),
            mpi_request,
        )
    ...
    def xchange_Euler_interfaces(
        self, geom: "CubedSphere", variables_itf_i: NDArray, variables_itf_j: NDArray, blocking: Optional[bool] = True
    ):
        ...
        X = self.device.array(
            geom.X[0, :]
        )  # TODO these should not be necessary (should already have device arrays in geometry)
        ...
        for do_flip, convert, positions, var, buffer in zip(
            ...
            buffer[id_first_tracer:] = (
                xp.flip(var[id_first_tracer:], flip_dim + 1) if do_flip else var[id_first_tracer:]
            )
    ...
    def xchange_vectors(
        ...
        if is_3d:
            if u3_itf_i is None:
                print(
                    f"Calling xchange_vectors with arrays that look like they are from a 3D problem, "
            ...
            if u3_itf_i is not None:
                print(
                    f"Calling xchange_vectors with arrays that look like they are from a 2D problem, "
        ...
        u1_n_recv, u2_n_recv, u3_n_recv = (
            get_rows(u1_itf_j, -1, 0),
            get_rows(u2_itf_j, -1, 0),
            get_rows(u3_itf_j, -1, 0),
        )
        ...
        u1_e_recv, u2_e_recv, u3_e_recv = (
            get_rows(u1_itf_i, -1, 0),
            get_rows(u2_itf_i, -1, 0),
            get_rows(u3_itf_i, -1, 0),
        )
        ...
        request = VectorNonBlockingExchangeRequest(
            ...
            (
                (u1_n_recv, u2_n_recv, u3_n_recv),
                (u1_s_recv, u2_s_recv, u3_s_recv),
                (u1_w_recv, u2_w_recv, u3_w_recv),
                (u1_e_recv, u2_e_recv, u3_e_recv),
            ),
            request,
            is_3d,
    ...
    def xchange_covectors(self, geom, u1_itf_i, u2_itf_i, u1_itf_j, u2_itf_j):
        ...
        if self.my_row == self.nb_lines_per_panel - 1:
            ...
            if self.my_panel == 0:
                ...
                sendbuf_u2[0, :] = u2_itf_j[-2, 1, :] + 2.0 * X / (1.0 + X**2) * u1_itf_j[-2, 1, :]

            elif self.my_panel == 1:

                sendbuf_u1[0, :] = -u2_itf_j[-2, 1, :] - 2.0 * X / (1.0 + X**2) * u1_itf_j[-2, 1, :]
                sendbuf_u2[0, :] = u1_itf_j[-2, 1, :]

            elif self.my_panel == 2:

                sendbuf_u1[0, :] = numpy.flipud(-u1_itf_j[-2, 1, :])
                sendbuf_u2[0, :] = numpy.flipud(-u2_itf_j[-2, 1, :] - 2.0 * X / (1.0 + X**2) * u1_itf_j[-2, 1, :])

            elif self.my_panel == 3:

                sendbuf_u1[0, :] = numpy.flipud(u2_itf_j[-2, 1, :] + 2.0 * X / (1.0 + X**2) * u1_itf_j[-2, 1, :])
        ...
        if self.my_row == 0:
            ...
            if self.my_panel == 0:
                ...
                sendbuf_u2[1, :] = u2_itf_j[1, 0, :] - 2.0 * X / (1.0 + X**2) * u1_itf_j[1, 0, :]

            elif self.my_panel == 1:

                sendbuf_u1[1, :] = numpy.flipud(u2_itf_j[1, 0, :] - 2.0 * X / (1.0 + X**2) * u1_itf_j[1, 0, :])
                sendbuf_u2[1, :] = numpy.flipud(-u1_itf_j[1, 0, :])

            elif self.my_panel == 2:

                sendbuf_u1[1, :] = numpy.flipud(-u2_itf_j[1, 0, :])
                sendbuf_u2[1, :] = numpy.flipud(-u2_itf_j[1, 0, :] + 2.0 * X / (1.0 + X**2) * u1_itf_j[1, 0, :])

            elif self.my_panel == 3:

                sendbuf_u1[1, :] = -u2_itf_j[1, 0, :] + 2.0 * X / (1.0 + X**2) * u1_itf_j[1, 0, :]
        ...
        if self.my_col == 0:
            ...
            if self.my_panel <= 3:
                ...
                sendbuf_u1[2, :] = -2.0 * Y / (1.0 + Y**2) * u2_itf_i[1, 0, :] + u1_itf_i[1, 0, :]
                sendbuf_u2[2, :] = u2_itf_i[1, 0, :]

            elif self.my_panel == 4:

                sendbuf_u1[2, :] = numpy.flipud(-u2_itf_i[1, 0, :])
                sendbuf_u2[2, :] = numpy.flipud(-2.0 * Y / (1.0 + Y**2) * u2_itf_i[1, 0, :] + u1_itf_i[1, 0, :])

            elif self.my_panel == 5:

                sendbuf_u1[2, :] = u2_itf_i[1, 0, :]
                sendbuf_u2[2, :] = 2.0 * Y / (1.0 + Y**2) * u2_itf_i[1, 0, :] - u1_itf_i[1, 0, :]
        ...
        if self.my_col == self.nb_elems_per_line - 1:
            ...
            if self.my_panel <= 3:
                ...
                sendbuf_u1[3, :] = 2.0 * Y / (1.0 + Y**2) * u2_itf_i[-2, 1, :] + u1_itf_i[-2, 1, :]
                sendbuf_u2[3, :] = u2_itf_i[-2, 1, :]

            elif self.my_panel == 4:

                sendbuf_u1[3, :] = u2_itf_i[-2, 1, :]
                sendbuf_u2[3, :] = -2.0 * Y / (1.0 + Y**2) * u2_itf_i[-2, 1, :] - u1_itf_i[-2, 1, :]

            elif self.my_panel == 5:

                sendbuf_u1[3, :] = numpy.flipud(-u2_itf_i[-2, 1, :])
                sendbuf_u2[3, :] = numpy.flipud(2.0 * Y / (1.0 + Y**2) * u2_itf_i[-2, 1, :] + u1_itf_i[-2, 1, :])
    ...
    def xchange_fluxes(
        ...
        if self.my_row == self.nb_lines_per_panel - 1:
            ...
            if self.my_panel == 0:
                ...
                sendbuf_T01[0, :] = T01_itf_j[-2, 1, :] - 2.0 * X / (1.0 + X**2) * T02_itf_j[-2, 1, :]
                sendbuf_T02[0, :] = T02_itf_j[-2, 1, :]
                sendbuf_T11[0, :] = (
                    T11_itf_j[-2, 1, :]
                    - 4.0 * X / (1.0 + X**2) * T12_itf_j[-2, 1, :]
                    + (2.0 * X / (1.0 + X**2)) ** 2 * T22_itf_j[-2, 1, :]
                )
                sendbuf_T12[0, :] = T12_itf_j[-2, 1, :] - 2.0 * X / (1.0 + X**2) * T22_itf_j[-2, 1, :]
                sendbuf_T22[0, :] = T22_itf_j[-2, 1, :]

            elif self.my_panel == 1:

                sendbuf_T01[0, :] = -T02_itf_j[-2, 1, :]
                sendbuf_T02[0, :] = T01_itf_j[-2, 1, :] - 2.0 * X / (1.0 + X**2) * T02_itf_j[-2, 1, :]
                ...
                sendbuf_T01[0, :] = numpy.flipud(-T01_itf_j[-2, 1, :] + 2.0 * X / (1.0 + X**2) * T02_itf_j[-2, 1, :])
                sendbuf_T02[0, :] = numpy.flipud(-T02_itf_j[-2, 1, :])
                sendbuf_T11[0, :] = numpy.flipud(
                    T11_itf_j[-2, 1, :]
                    - 4.0 * X / (1.0 + X**2) * T12_itf_j[-2, 1, :]
                    + (2.0 * X / (1.0 + X**2)) ** 2 * T22_itf_j[-2, 1, :]
                )
                sendbuf_T12[0, :] = numpy.flipud(T12_itf_j[-2, 1, :] - 2.0 * X / (1.0 + X**2) * T22_itf_j[-2, 1, :])
                sendbuf_T22[0, :] = numpy.flipud(T22_itf_j[-2, 1, :])

            elif self.my_panel == 3:

                sendbuf_T01[0, :] = numpy.flipud(T02_itf_j[-2, 1, :])
                sendbuf_T02[0, :] = numpy.flipud(-T01_itf_j[-2, 1, :] + 2.0 * X / (1.0 + X**2) * T02_itf_j[-2, 1, :])
                sendbuf_T11[0, :] = numpy.flipud(T22_itf_j[-2, 1, :])
                sendbuf_T12[0, :] = numpy.flipud(-T12_itf_j[-2, 1, :] + 2.0 * X / (1.0 + X**2) * T22_itf_j[-2, 1, :])
                sendbuf_T22[0, :] = numpy.flipud(
                    T11_itf_j[-2, 1, :]
                    - 4.0 * X / (1.0 + X**2) * T12_itf_j[-2, 1, :]
                    + (2.0 * X / (1.0 + X**2)) ** 2 * T22_itf_j[-2, 1, :]
                )
        ...
        if self.my_row == 0:
            ...
            if self.my_panel == 0:
                ...
                sendbuf_T01[1, :] = T01_itf_j[1, 0, :] + 2.0 * X / (1.0 + X**2) * T02_itf_j[1, 0, :]
                sendbuf_T02[1, :] = T02_itf_j[1, 0, :]
                sendbuf_T11[1, :] = (
                    T11_itf_j[1, 0, :]
                    + 4.0 * X / (1.0 + X**2) * T12_itf_j[1, 0, :]
                    + (2.0 * X / (1.0 + X**2)) ** 2 * T22_itf_j[1, 0, :]
                )
                sendbuf_T12[1, :] = T12_itf_j[1, 0, :] + 2.0 * X / (1.0 + X**2) * T22_itf_j[1, 0, :]
                ...
                sendbuf_T22[1, :] = numpy.flipud(
                    T11_itf_j[1, 0, :]
                    + 4 * X / (1 + X**2) * T12_itf_j[1, 0, :]
                    + (2 * X / (1 + X**2)) ** 2 * T22_itf_j[1, 0, :]
                )

            elif self.my_panel == 2:

                sendbuf_T01[1, :] = numpy.flipud(-T01_itf_j[1, 0, :] - 2.0 * X / (1.0 + X**2) * T02_itf_j[1, 0, :])
                sendbuf_T02[1, :] = numpy.flipud(-T02_itf_j[1, 0, :])
                sendbuf_T11[1, :] = numpy.flipud(
                    T11_itf_j[1, 0, :]
                    + 4.0 * X / (1.0 + X**2) * T12_itf_j[1, 0, :]
                    + (2.0 * X / (1.0 + X**2)) ** 2 * T22_itf_j[1, 0, :]
                )
                sendbuf_T12[1, :] = numpy.flipud(T12_itf_j[1, 0, :] + 2.0 * X / (1.0 + X**2) * T22_itf_j[1, 0, :])
                sendbuf_T22[1, :] = numpy.flipud(T22_itf_j[1, 0, :])

            elif self.my_panel == 3:

                sendbuf_T01[1, :] = -T02_itf_j[1, 0, :]
                sendbuf_T02[1, :] = T01_itf_j[1, 0, :] + 2.0 * X / (1.0 + X**2) * T02_itf_j[1, 0, :]
        ...
        if self.my_col == 0:
            ...
            if self.my_panel <= 3:
                ...
                sendbuf_T02[2, :] = 2.0 * Y / (1 + Y**2) * T01_itf_i[1, 0, :] + T02_itf_i[1, 0, :]
                sendbuf_T11[2, :] = T11_itf_i[1, 0, :]
                sendbuf_T12[2, :] = 2.0 * Y / (1 + Y**2) * T11_itf_i[1, 0, :] + T12_itf_i[1, 0, :]
                sendbuf_T22[2, :] = (
                    (2.0 * Y / (1 + Y**2)) ** 2 * T11_itf_i[1, 0, :]
                    + 4.0 * Y / (1.0 + Y**2) * T12_itf_i[1, 0, :]
                    + T22_itf_i[1, 0, :]
                )

            elif self.my_panel == 4:

                sendbuf_T01[2, :] = numpy.flipud(-2.0 * Y / (1.0 + Y**2) * T01_itf_i[1, 0, :] - T02_itf_i[1, 0, :])
                sendbuf_T02[2, :] = numpy.flipud(T01_itf_i[1, 0, :])
                sendbuf_T11[2, :] = numpy.flipud(
                    (2.0 * Y / (1.0 + Y**2)) ** 2 * T11_itf_i[1, 0, :]
                    + 4.0 * Y / (1.0 + Y**2) * T12_itf_i[1, 0, :]
                    + T22_itf_i[1, 0, :]
                )
                sendbuf_T12[2, :] = numpy.flipud(-2.0 * Y / (1.0 + Y**2) * T11_itf_i[1, 0, :] - T12_itf_i[1, 0, :])
                sendbuf_T22[2, :] = numpy.flipud(T11_itf_i[1, 0, :])

            elif self.my_panel == 5:

                sendbuf_T01[2, :] = 2.0 * Y / (1.0 + Y**2) * T01_itf_i[1, 0, :] + T02_itf_i[1, 0, :]
                ...
                sendbuf_T12[2, :] = -2.0 * Y / (1.0 + Y**2) * T11_itf_i[1, 0, :] - T12_itf_i[1, 0, :]
        ...
        if self.my_col == self.nb_elems_per_line - 1:
            ...
            if self.my_panel <= 3:
                ...
                sendbuf_T22[3, :] = (
                    (2 * Y / (1 + Y**2)) ** 2 * T11_itf_i[-2, 1, :]
                    - 4.0 * Y / (1.0 + Y**2) * T12_itf_i[-2, 1, :]
                    + T22_itf_i[-2, 1, :]
                )
                ...
                sendbuf_T01[3, :] = numpy.flipud(2.0 * Y / (1.0 + Y**2) * T01_itf_i[-2, 1, :] - T02_itf_i[-2, 1, :])
                sendbuf_T02[3, :] = numpy.flipud(T01_itf_i[-2, 1, :])
                sendbuf_T11[3, :] = numpy.flipud(
                    (2.0 * Y / (1.0 + Y**2)) ** 2 * T11_itf_i[-2, 1, :]
                    - 4.0 * Y / (1.0 + Y**2) * T12_itf_i[-2, 1, :]
                    + T22_itf_i[-2, 1, :]
                )
                sendbuf_T12[3, :] = numpy.flipud(2.0 * Y / (1.0 + Y**2) * T11_itf_i[-2, 1, :] - T12_itf_i[-2, 1, :])
