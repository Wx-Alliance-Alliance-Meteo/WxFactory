...
def image_field(
    ...
    if not geom.xperiodic:
        cmap = matplotlib.pyplot.contourf(
            geom.X1, geom.X3, field, cmap=colormap, levels=numpy.linspace(vmin, vmax, n), extend="both"
        )
    else:
        ...
        cmap = matplotlib.pyplot.contourf(X1, X3, f, cmap=colormap, levels=numpy.linspace(vmin, vmax, n), extend="both")
