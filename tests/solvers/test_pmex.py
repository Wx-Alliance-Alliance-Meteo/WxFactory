...
class PmexTestCases(cuda_test.CudaTestCases):
    ...
    def test_compare_cpu_to_gpu(self):
        ...
        w1_value: float = self.cpu_device.xp.sum(w1).item()
        ...
        if self.show_debug_print:
            print(f'In PmexTestCases.test_compare_cpu_to_gpu, relative difference from cpu is {relative_diff_w1} and relative difference from gpu is {relative_diff_w2}')

        self.assertLessEqual(relative_diff_w1, self.cpu_gpu_tolenrance, f'Pmex didn\'t give a close result to the cpu value')
        self.assertLessEqual(relative_diff_w2, self.cpu_gpu_tolenrance, f'Pmex didn\'t give a close result to the gpu value')
