...
class KiopsPmexToleranceGpuTestCases(cuda_test.CudaTestCases):
    ...
    def setUp(self) -> None:
        ...
        self.pmex_matrix: ndarray = self.gpu_device.xp.zeros((initial_matrix_size, initial_matrix_size), dtype=float)
    ...
    def test_compare_kiops_pmex(self):
        ...
        w1, stats1 = kiops([1.], matvec_handle, self.kiops_matrix, self.tolerance, device=self.gpu_device)
        w2, stats2 = pmex([1.], matvec_handle, self.pmex_matrix, self.tolerance, device=self.gpu_device)
        ...
        if self.show_debug_print:
            c_name = 'KiopsPmexToleranceGpuTestCases'
