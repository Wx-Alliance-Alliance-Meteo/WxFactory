"""Check the forward-mode RHS derivative across cubed-sphere halo exchanges.
...
def global_norm(x: torch.Tensor) -> float:
    return comm.allreduce(float(torch.sum(x * x, dtype=torch.float64))) ** 0.5
...
process_topology._EXCHANGE_TANGENTS = False
try:
    with fwad.dual_level():
        without = fwad.unpack_dual(rhs(fwad.make_dual(Q, v))).tangent.clone()
finally:
    process_topology._EXCHANGE_TANGENTS = True
