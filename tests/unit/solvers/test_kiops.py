...
class KiopsComparisonTestCases(cuda_test.CudaTestCases):
    ...
    def test_compare_cpu_to_gpu(self):
        ...
        w1, stats1 = kiops([1.0], matvec_handle, self.cpu_matrix, self.tolerance, device=self.cpu_device)
        w2, stats2 = kiops([1.0], matvec_handle, self.gpu_matrix, self.tolerance, device=self.gpu_device)
