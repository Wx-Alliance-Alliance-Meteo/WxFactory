...
class PDEPointwiseFluxGenericTestCase(MpiTestCase):
    ...
    def rel_diff(self, a: NDArray, b: NDArray, xp) -> float:
        vars = []
        for i in range(a.shape[0]):
            vars.append(xp.linalg.norm(b[i] - a[i]))
            ref_norm = xp.linalg.norm(a[i])
            if ref_norm > 0.0:
                vars[i] /= ref_norm
    ...
    def test_pointwise_flux_kernel(self, device: str):
        ...
        for state_file in state_files:
            ...
            (
                ...
                wflux_adv_x2_py,
                wflux_adv_x3_py,
            ) = [xp.zeros(pressure_shape, dtype=float) for _ in range(8)]
            ...
            sim.rhs.full.pde.pointwise_fluxes_py(
                local_state,
                flux_x1_py,
                flux_x2_py,
                flux_x3_py,
            ...
            sim.rhs.full.pde.pointwise_fluxes_code(
                local_state,
                flux_x1_code,
                flux_x2_code,
                flux_x3_code,
