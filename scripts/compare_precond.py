...
def print_result(jacobian, rhs, sol, num_iter, solve_time, prefix=""):
    ...
    print(
        f"{prefix:15s} -- Relative error: {rel_error:.3e}, absolute: {abs_error:.3e}, "
...
def solve_gef_fgmres(jacobian, rhs, precond=None):
    ...
    result, solve_time = timed_call(
        partial(linsol.fgmres, matvec, rhs, restart=20, maxiter=25, tol=tolerance, preconditioner=precond)
    )
...
def solve_pyamg_krylov(solver, jacobian, rhs, x0=None, precond=None):
    ...
    result, t = timed_call(
        partial(solver, jacobian, rhs, tol=tolerance, x0=x0, maxiter=500, M=precond, callback=pyamg_callback)
    )
