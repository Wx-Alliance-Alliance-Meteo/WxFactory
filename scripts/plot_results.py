...
def get_linestyle(data: DataSet, index: int):
    ...
    if data["precond"] == "none":
        return main_linestyles[index % len(main_linestyles)]
    return None
...
def get_marker(data: DataSet) -> Union[str, int, None]:
    ...
    if data["precond"] == "fv-mg":
        if data["mg_smoother"] == "ark3":
            return "2"
        return "."
    if data["precond"] == "p-mg":
        return "x"
    return None


def has_diff(
    datasets: List[SingleResultSet], params: List[str], conditions: Callable[[DataSet], bool] = lambda d: True
) -> bool:
    ...
    for dataset in datasets:
        ...
        for data in dataset:
            if conditions(data):
                ...
                if val is None:
                    val = current
                elif any(a != b for a, b in zip(val, current)):
                    return True
...
class FullDataSet:
    ...
    def __init__(
        ...
        time_condition = []
        if prob.dt > 0.0:
            time_condition += [f"initial_dt = {prob.dt}"]
        ...
        self.fv_mg += self._extract_result_set(
            ...
            time_condition
            + [f'precond = "fv-mg"', 'mg_smoother = "exp"', "exp_radius_0 > 0", "mg_solve_coarsest = 1"]
            + pcfl_condition,
            num_best=2,
        )
        ...
        self.same_fv_pseudo_cfl = not has_diff(
            [self.fv_mg], ["pseudo_cfl"], conditions=lambda d: d["mg_smoother"] in ["erk1", "erk3", "ark3"]
        )
        self.same_fv_num_smoothe = not has_diff(
            [self.fv_mg], ["num_pre_smoothe", "num_post_smoothe", "mg_solve_coarsest"]
        )
        ...
        if self.same_fv_num_smoothe:
            self.fv_num_smoothe = (
                f'{self.fv_mg[0]["num_pre_smoothe"]}/'
    ...
    def _find_first(self, param: str) -> Any:
        if len(self.no_precond) > 0:
            return self.no_precond[0][param]
        if len(self.fv_ref) > 0:
            return self.fv_ref[0][param]
        if len(self.fv_mg) > 0:
            return self.fv_mg[0][param]
        if len(self.p_mg) > 0:
            return self.p_mg[0][param]
        return None
    ...
    def _make_label(self, data: Dict[str, Any]) -> str:
        ...
        elements = []
        if data["precond"] == "none":
            elements.append(data["time_integrator"])
        elif data["precond"] == "fv":
            elements.append(data["precond"])
        else:
            if self.same_mg_precond:
                elements.append("mg")
            else:
                elements.append(data["precond"])
        if not self.same_tol:
            elements.append(f'tol {data["solver_tol"]:.0e}')
        if not self.same_dt:
            elements.append(f'dt {data["initial_dt"]}')
        ...
        if data["precond"] == "fv-mg":
            if not self.same_fv_smoother:
                elements.append(f'{data["mg_smoother"]}')
            if data["mg_smoother"] in ["erk1", "erk3", "ark3"]:
                if not self.same_fv_pseudo_cfl:
                    cfl = data["pseudo_cfl"]
                    if cfl >= 10000.0:
                        elements.append(f"pdt {cfl/1000.0:.0f}k")
                    elif cfl >= 100.0:
                        elements.append(f"pdt {cfl:5.0f}")
                    else:
                        elements.append(f"pdt {cfl:5.2f}")
    ...
    def _make_title(self, base_title: str) -> str:
        ...
        if self.same_dt:
            items += [f"dt = {self.dt}"]
        if self.same_tol:
            items += [f"tol = {self.tol:.0e}"]
        title += ", ".join(items)
        title += " || "
        if self.same_mg_precond:
            title += f"MG precond: {self.mg_precond}"
            if self.same_fv_smoother:
                title += f", smoother {self.fv_smoother}"
            if self.same_fv_num_smoothe:
                title += f", #sm {self.fv_num_smoothe}"
            if self.same_fv_precond_tol and self.fv_precond_tol is not None:
                title += f", tol {self.fv_precond_tol}"
            if self.same_fv_pseudo_cfl:
                cfl = self.fv_pseudo_cfl
                if cfl >= 10000.0:
                    title += f", pdt {cfl/1000.0:.0f}k"
                elif cfl >= 100.0:
                    title += f", pdt {cfl:5.0f}"
                else:
                    title += f", pdt {cfl:5.2f}"
    ...
    def _extract_result_set(
        self, columns: List[str], conditions: List[str], debug: bool = False, num_best: int = 20
    ) -> SingleResultSet:
        """
    ...
    def plot_time_per_step(self):
        ...
        for dataset in [self.no_precond, self.fv_ref, self.p_mg, self.fv_mg]:
            for i, data in enumerate(dataset):
                ...
                ax.plot(
                    data["sim_times"] + data["step_dts"],
                    data["time_per_step"] / data["step_dts"],
    ...
    def plot_error_time(self):
        ...
        fig.suptitle(
            self._make_title(f"Time (work) to reach accuracy"), fontsize=10, x=0.06, horizontalalignment="left"
        )
        fig.legend(fontsize=8)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--it", action="store_true", help="Plot the number of iterations needed to reach target residual"
    )
    parser.add_argument(
        "--error-time", action="store_true", help="Plot time needed to reach a certain error (residual) level"
    )
    parser.add_argument(
        "--time-per-step",
        action="store_true",
    ...
    parser.add_argument(
        "--split-dt", action="store_true", help="Whether to split plots per dt (rather than all dt's on the same plot)"
    )
    ...
    parser.add_argument(
        "--pseudo-cfl",
        type=str,
        help="Additional condition to put on the value of pseudo-CFL we want to include in plots",
    )
