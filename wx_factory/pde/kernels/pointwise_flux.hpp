...
pointwise_euler_cubedsphere_3d_kernel(kernel_params_cubedsphere<real_t, num_t> params, bool verbose) {
  ...
  *params.flux[0].rho_v     = sqrt_g * (rho_u * v + params.h[h12] * p);
  *params.flux[0].rho_w     = sqrt_g * (rho_u * w + params.h[h13] * p);
  *params.flux[0].rho_theta = sqrt_g * rho_theta * u;

  *params.flux[1].rho       = sqrt_g * rho_v;
  *params.flux[1].rho_u     = sqrt_g * (rho_v * u + params.h[h21] * p);
  *params.flux[1].rho_v     = sqrt_g * (rho_v * v + params.h[h22] * p);
  *params.flux[1].rho_w     = sqrt_g * (rho_v * w + params.h[h23] * p);
  *params.flux[1].rho_theta = sqrt_g * rho_theta * v;

  *params.flux[1].rho       = sqrt_g * rho_w;
  *params.flux[1].rho_u     = sqrt_g * (rho_w * u + params.h[h31] * p);
  *params.flux[1].rho_v     = sqrt_g * (rho_w * v + params.h[h32] * p);
  *params.flux[1].rho_w     = sqrt_g * (rho_w * w + params.h[h33] * p);
  *params.flux[1].rho_theta = sqrt_g * rho_theta * w;
