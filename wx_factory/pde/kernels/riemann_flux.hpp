...
DEVICE_SPACE void riemann_euler_cubedsphere_rusanov_3d_kernel(
  ...
  *params_l.flux[dir].rho       = 0.5 * (flux_l[0] + flux_r[0] - scaled_eig * (rhor - rhol));
  *params_l.flux[dir].rho_u     = 0.5 * (flux_l[1] + flux_r[1] - scaled_eig * (rho_ur - rho_ul));
  *params_l.flux[dir].rho_v     = 0.5 * (flux_l[2] + flux_r[2] - scaled_eig * (rho_vr - rho_vl));
  *params_l.flux[dir].rho_w     = 0.5 * (flux_l[3] + flux_r[3] - scaled_eig * (rho_wr - rho_wl));
  *params_l.flux[dir].rho_theta = 0.5 * (flux_l[4] + flux_r[4] - scaled_eig * (rho_thetar - rho_thetal));
  ...
  *params_l.wflux_adv[dir] = 0.5 * (rho_wl * vnl + rho_wr * vnr - scaled_eig * (rho_wr - rho_wl));
  *params_r.wflux_adv[dir] = *params_l.wflux_adv[dir];
  ...
  *params_l.wflux_pres[dir] = wflux_pres_avg / pl;
  *params_r.wflux_pres[dir] = wflux_pres_avg / pr;
}