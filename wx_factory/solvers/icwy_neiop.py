...
"""
 ---> incomplete ICWY with norm estimate 
...
def icwy_neiop(τ_out, A, u, tol=1e-7, m_init=10, mmin=10, mmax=128, iop=2, task1=False):
    ...
    while τ_now < τ_end:
        ...
        while j < m:
            ...
            # for some reason, how temp is computed leads to an issue with the method
            # need to first do 0:n elements, then in clude n:n+p, or else
            # there is a NaN after timestep 1

            # 1. Compute values for ICWY
            indx = max(0, j - iop)
            # temp = V[indx:j+1, :] @ V[j-1:j+1, :].T #<--method fails with this
            temp = V[indx : j + 1, 0:n] @ V[j - 1 : j + 1, 0:n].T

            global_temp = numpy.empty_like(temp)
            mpi4py.MPI.COMM_WORLD.Allreduce([temp, mpi4py.MPI.DOUBLE], [global_temp, mpi4py.MPI.DOUBLE])
            global_temp += V[indx : j + 1, n : n + p].dot(numpy.transpose(V[j - 1 : j + 1, n : n + p]))

            # 2. Build matrix T
            if j > 1:
                # for T matrix, which is negative values from T
                # this holds the loss of orthogonality part
                T[1, 0] = -global_temp[0, 0]
                tmp = T @ global_temp[0:iop, 1]
                V[j, :] -= tmp @ V[indx:j, :]

            else:
                tmp = T[0, 0] * global_temp[0, 1]
                V[j, :] -= V[0, :].dot(tmp)

            # 6. norm estimate
            indx2 = min(1, j - 1)
            sum_sqrd = numpy.sum(global_temp[0 : indx2 + 1, 1] ** 2)

            if global_temp[-1, 1] < sum_sqrd:
                local_sum = V[j, 0:n] @ V[j, 0:n]
                nrmish = numpy.sqrt(mpi4py.MPI.COMM_WORLD.allreduce(local_sum) + V[j, n : n + p] @ V[j, n : n + p])
                reg_comm += 1
            else:
                nrmish = math.sqrt(global_temp[-1, 1] - sum_sqrd)
