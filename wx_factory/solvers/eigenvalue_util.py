...
def load_matrices(file):
    ...
    while True:
        ...
        if matrix_format in ("csc", "csr", "bsr"):
            matrices.append(
                cls(
                    (loaded[f"data{num_loaded}"], loaded[f"indices{num_loaded}"], loaded[f"indptr{num_loaded}"]),
                    shape=loaded[f"shape{num_loaded}"],
                )
            )
        elif matrix_format == "dia":
            matrices.append(
                cls((loaded[f"data{num_loaded}"], loaded[f"offsets{num_loaded}"]), shape=loaded[f"shape{num_loaded}"])
            )
        elif matrix_format == "coo":
            matrices.append(
                cls(
                    (loaded[f"data{num_loaded}"], (loaded[f"row{num_loaded}"], loaded[f"col{num_loaded}"])),
                    shape=loaded[f"shape{num_loaded}"],
                )
            )
        else:
