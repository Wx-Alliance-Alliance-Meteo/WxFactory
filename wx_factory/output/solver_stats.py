...
class SolverStatsOutput:
    ...
    def _exec_write_output(
        ...
        if (
            hasattr(self.param, "exp_smoothe_spectral_radii")
            and len(self.param.exp_smoothe_spectral_radii) > 0
            and precond is not None
        ):
            if len(precond.spectral_radii) > 0:
                self.columns.exp_radius_0.value = precond.spectral_radii[0]
            if len(precond.spectral_radii) > 1:
                self.columns.exp_radius_1.value = precond.spectral_radii[1]
            if len(precond.spectral_radii) > 2:
                self.columns.exp_radius_2.value = precond.spectral_radii[2]
            if len(precond.spectral_radii) > 3:
                self.columns.exp_radius_3.value = precond.spectral_radii[3]
            if len(precond.spectral_radii) > 4:
