...
from common.definitions import idx_2d_rho as RHO, idx_2d_rho_w as RHO_W, idx_2d_rho_theta as RHO_THETA
...
def output_step(Q: numpy.ndarray, geom: Cartesian2D, param: Configuration, filename: str) -> None:
    ...
    if param.case_number == 0:
        ...
        image_field(geom, (Q_cartesian[RHO_THETA, :, :] / Q_cartesian[RHO, :, :]), filename, 303.0, 303.7, 8)
    elif param.case_number == 4:
        image_field(geom, (Q_cartesian[RHO_THETA, :, :] / Q_cartesian[RHO, :, :]), filename, 290.0, 300.0, 10)
