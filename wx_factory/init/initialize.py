...
def conservative_to_entropy(Q: NDArray) -> NDArray[numpy.float64]:
    """ Compute entropy variables V(Q) = dS/dQ from the conservative variables"""
    num_equations = 4
    xp = geom.device.xp
...
def entropy_to_conservative(V: NDArray) -> NDArray[numpy.float64]:
    ...
    # Check this formula!!!!
    ρ_e = ((gamma - 1)/ (-v4)**gamma)**(1/(gamma-1)) * np.exp(-s/(gamma - 1) )
