...
def eval_single_field_3d(field, elem_interp, xp, result=None):
    ...
    interp_1 = xp.empty_like(
        field, shape=(num_elem_vert * old_num_points, num_elem_horiz * new_num_points, num_elem_horiz * old_num_points)
    )
    interp_2 = xp.empty_like(
        field, shape=(num_elem_vert * old_num_points, num_elem_horiz * new_num_points, num_elem_horiz * new_num_points)
    )
    if result is None:
        result = xp.empty_like(
            field,
            shape=(num_elem_vert * new_num_points, num_elem_horiz * new_num_points, num_elem_horiz * new_num_points),
        )
...
def lagrange_poly(index, order):
    ...
    def L(x):
        ...
        """
        return numpy.prod([(x - points[i]) / (points[index] - points[i]) for i in range(order) if i != index], axis=0)
...
def get_basis_points(basis_type: str, order: int, include_boundary: bool = False) -> numpy.ndarray:
    ...
    if basis_type == "dg":
        ...
        if include_boundary:
            points = numpy.append(-1.0, numpy.append(points, 1.0))
    elif basis_type == "fv":
