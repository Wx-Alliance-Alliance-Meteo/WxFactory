...
DEVICE_SPACE
static constexpr real_t extrap_factors[][6] = {
...
struct extrap_x_kernel
{
  ...
  operator()(extrap_params_cubedsphere<num_t, order> params, const bool verbose) {
    ...
    {
      *params.side_x1 = params.elem[0];
      *params.side_x2 = params.elem[0];
      return;
    }

    const size_t offset = params.index % (order * order);
    num_t        side1(0.0);
    num_t        side2(0.0);
    for (int i = 0; i < order; i++)
    {
      side1 += params.elem[offset * order + i] * extrap_factors<real_t>[order - 2][i];
      side2 += params.elem[offset * order + i] *
               extrap_factors<real_t>[order - 2][order - i - 1];
    }
    *params.side_x1 = side1;
    *params.side_x2 = side2;
  }
};

template <typename real_t, typename num_t, int order>
struct extrap_y_kernel
{
  DEVICE_SPACE void
  operator()(extrap_params_cubedsphere<num_t, order> params, const bool verbose) {
    ...
    if constexpr (order == 1)
    {
      *params.side_y1 = params.elem[0];
      *params.side_y2 = params.elem[0];
      return;
    }

    const size_t o2     = order * order;
    const size_t rem    = params.index % o2;
    const size_t offset = rem % order + (rem / order) * o2;
    num_t        side1(0.0);
    num_t        side2(0.0);
    for (int i = 0; i < order; i++)
    {
      ...
      // {
      //   printf(
      //       "%2ld[%d]: side1 += elem[%2ld] (%f) * %f\n",
      //       params.index,
      //       i,
      //       offset + order * i,
      //       to_real(params.elem[offset + order * i]),
      //       extrap_factors<real_t>[order - 2][i]);
      // }
    }
    *params.side_y1 = side1;
    *params.side_y2 = side2;
  }
};

template <typename real_t, typename num_t, int order>
struct extrap_z_kernel
{
  DEVICE_SPACE void
  operator()(extrap_params_cubedsphere<num_t, order> params, const bool verbose) {
    ...
    if constexpr (order == 1)
    {
      *params.side_z1 = params.elem[0];
      *params.side_z2 = params.elem[0];
      return;
    }

    const size_t o2     = order * order;
    const size_t offset = params.index % (o2);
    num_t        side1(0.0);
    num_t        side2(0.0);
    for (int i = 0; i < order; i++)
    {
      side1 += params.elem[offset + o2 * i] * extrap_factors<real_t>[order - 2][i];
      side2 +=
          params.elem[offset + o2 * i] * extrap_factors<real_t>[order - 2][order - i - 1];
    }
    *params.side_z1 = side1;
    *params.side_z2 = side2;
  }
};
