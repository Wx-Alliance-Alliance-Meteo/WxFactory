...
inline HOST_DEVICE_SPACE size_t compute_itf_offset(const size_t index, const size_t order) {
...
struct extrap_params_cubedsphere
{
  ...
  DEVICE_SPACE void set_index(const size_t new_index) {
    ...
    const int64_t offset_diff = static_cast<int64_t>(new_index % itf_size) - old_index % itf_size;
