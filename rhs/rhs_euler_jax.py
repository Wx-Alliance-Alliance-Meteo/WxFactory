...
from common.definitions import idx_rho_u1, idx_rho_u2, idx_rho_w, idx_rho, idx_rho_theta, gravity, \
                               p0, Rd, cpd, cvd, heat_capacity_ratio
...
#@profile
def rhs_euler_jax(Q: NDArray, geom: CubedSphere, mtrx: DFROperators, metric: Metric3DTopo, ptopo: DistributedWorld,
   ...
   df1_dx1, df2_dx2, df3_dx3, rhs = [jnp.empty_like(Q, dtype=type_vec) for _ in range(4)]

   # Array for forcing: Coriolis terms, metric corrections from the curvilinear coordinate, and gravity
   forcing = jnp.zeros_like(Q, dtype=type_vec)

   # Array to extrapolate variables and fluxes to the boundaries along x (i)
   variables_itf_i = jnp.ones((nb_equations, nb_vertical_levels, nb_elements_hori + 2, 2, nb_pts_hori), dtype=type_vec) # Initialized to one in the halo to avoid division by zero later
   # Note that flux_x1_itf_i has a different shape than variables_itf_i
   flux_x1_itf_i   = jnp.empty((nb_equations, nb_vertical_levels, nb_elements_hori + 2, nb_pts_hori, 2), dtype=type_vec)

   # Extrapolation arrays along y (j)
   variables_itf_j = jnp.ones((nb_equations, nb_vertical_levels, nb_elements_hori + 2, 2, nb_pts_hori), dtype=type_vec) # Initialized to one in the halo to avoid division by zero later
   flux_x2_itf_j   = jnp.empty((nb_equations, nb_vertical_levels, nb_elements_hori + 2, 2, nb_pts_hori), dtype=type_vec)

   # Extrapolation arrays along z (k), note dimensions of (6, nj, nk+2, 2, ni)
   variables_itf_k = jnp.empty((nb_equations, nb_pts_hori, nb_elements_vert + 2, 2, nb_pts_hori), dtype=type_vec)
   flux_x3_itf_k   = jnp.empty((nb_equations, nb_pts_hori, nb_elements_vert + 2, 2, nb_pts_hori), dtype=type_vec)

   # Special arrays for calculation of (ρw) flux
   wflux_adv_x1_itf_i = jnp.zeros_like(flux_x1_itf_i[0,:])
   wflux_pres_x1_itf_i = jnp.zeros_like(wflux_adv_x1_itf_i)
   wflux_adv_x2_itf_j = jnp.zeros_like(flux_x2_itf_j[0,:])
   wflux_pres_x2_itf_j = jnp.zeros_like(wflux_adv_x2_itf_j)
   wflux_adv_x3_itf_k = jnp.zeros_like(flux_x3_itf_k[0,:])
   wflux_pres_x3_itf_k = jnp.zeros_like(flux_x3_itf_k[0,:])
   ...
   #    epais = elem * nbsolpts + jnp.arange(nbsolpts)
   ...
   variables_itf_i.at[:,:,1:-1,:,:].set(mtrx.extrapolate_i(Q,geom).transpose((0,1,3,4,2)))
   variables_itf_j.at[:,:,1:-1,:,:].set(mtrx.extrapolate_j(Q,geom))

   # Scaled variables for separate reconstruction
   logrho = jnp.log(Q[idx_rho,:])
   logrhotheta = jnp.log(Q[idx_rho_theta,:])

   variables_itf_i.at[idx_rho,:,1:-1,:,:].set(jnp.exp(mtrx.extrapolate_i(logrho,geom)).transpose((0,2,3,1)))
   variables_itf_j.at[idx_rho,:,1:-1,:,:].set(jnp.exp(mtrx.extrapolate_j(logrho,geom)))

   variables_itf_i.at[idx_rho_theta,:,1:-1,:,:].set(jnp.exp(mtrx.extrapolate_i(logrhotheta,geom)).transpose((0,2,3,1)))
   variables_itf_j.at[idx_rho_theta,:,1:-1,:,:].set(jnp.exp(mtrx.extrapolate_j(logrhotheta,geom)))
   ...
   pressure = p0 * jnp.exp((cpd/cvd) * jnp.log((Rd/p0)*Q[idx_rho_theta]))
   #pressure = Rd * Q[idx_rho_theta]

   wflux_pres_x1 = jnp.zeros(metric.sqrtG.shape,dtype=type_vec)
   wflux_pres_x2 = jnp.zeros(metric.sqrtG.shape,dtype=type_vec)
   wflux_pres_x3 = jnp.zeros(metric.sqrtG.shape,dtype=type_vec)
   ...
   variables_itf_k[idx_rho,:,1:-1,:,:] = jnp.exp(mtrx.extrapolate_k(logrho,geom).transpose((2,0,1,3)))
   variables_itf_k[idx_rho_theta,:,1:-1,:,:] = jnp.exp(mtrx.extrapolate_k(logrhotheta,geom).transpose((2,0,1,3)))
   ...
   pressure_itf_k = p0 * jnp.exp((cpd/cvd)*jnp.log(variables_itf_k[idx_rho_theta] * (Rd / p0)))
   ...
   # flux_D_itf_k = jnp.empty((nb_equations, nb_interfaces_vert, geom.nj, geom.ni))
   # flux_U_itf_k = jnp.empty((nb_equations, nb_interfaces_vert, geom.nj, geom.ni))
   # eig_D_itf_k = jnp.empty((nb_interfaces_vert, geom.nj, geom.ni))
   # eig_U_itf_k = jnp.empty((nb_interfaces_vert, geom.nj, geom.ni))
   for itf in range(nb_interfaces_vert):
      ...
      if advection_only: # Eigenvalues are simply the advection speeds
         ...
         eig_D = jnp.abs(w_D) + jnp.sqrt(metric.H_contra_33_itf_k[itf,:,:] * heat_capacity_ratio * \
                                                pressure_itf_k[:, elem_D, 1, :] / variables_itf_k[idx_rho, :, elem_D, 1, :])
         eig_U = jnp.abs(w_U) + jnp.sqrt(metric.H_contra_33_itf_k[itf,:,:] * heat_capacity_ratio * \
                                                pressure_itf_k[:, elem_U, 0, :] / variables_itf_k[idx_rho, :, elem_U, 0, :])

      eig = jnp.maximum(eig_D, eig_U)
   ...
   pressure_itf_i = p0 * jnp.exp((cpd/cvd) * jnp.log(variables_itf_i[idx_rho_theta] * (Rd / p0)))
   pressure_itf_j = p0 * jnp.exp((cpd/cvd) * jnp.log(variables_itf_j[idx_rho_theta] * (Rd / p0)))
   ...
   for itf in range(nb_interfaces_hori):
      ...
      if advection_only: # Advection only, eigenvalues are the velocities alone
         ...
         eig_L = jnp.abs( u1_L ) + jnp.sqrt(metric.H_contra_11_itf_i[:,:,itf] * heat_capacity_ratio * pressure_itf_i[:, elem_L, 1, :] / variables_itf_i[idx_rho, :, elem_L, 1, :])
         eig_R = jnp.abs( u1_R ) + jnp.sqrt(metric.H_contra_11_itf_i[:,:,itf] * heat_capacity_ratio * pressure_itf_i[:, elem_R, 0, :] / variables_itf_i[idx_rho, :, elem_R, 0, :])

      eig = jnp.maximum(eig_L, eig_R)
      ...
      if advection_only:
         ...
         eig_L = jnp.abs( u2_L ) + jnp.sqrt(metric.H_contra_22_itf_j[:, itf, :] * heat_capacity_ratio * pressure_itf_j[:, elem_L, 1, :] / variables_itf_j[idx_rho, :, elem_L, 1, :])
         eig_R = jnp.abs( u2_R ) + jnp.sqrt(metric.H_contra_22_itf_j[:, itf, :]  * heat_capacity_ratio * pressure_itf_j[:, elem_R, 0, :] / variables_itf_j[idx_rho, :, elem_R, 0, :])
   ...
   logp_bdy_i = jnp.log(pressure_bdy_i)
   logp_bdy_j = jnp.log(pressure_bdy_j)
   logp_bdy_k = jnp.log(pressure_bdy_k)
   ...
   forcing[idx_rho_w] = 2.0 * (metric.christoffel_3_01 * rho * u1 + metric.christoffel_3_02 * rho * u2 + metric.christoffel_3_03 * rho * w) \
         ...
         #+ metric.inv_dzdeta * gravity * jnp.exp(mtrx.filter_k(logrho, geom))
