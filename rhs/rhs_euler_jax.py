...
#@profile
def rhs_euler_jax(Q: numpy.ndarray, geom: CubedSphere, mtrx: DFROperators, metric: Metric3DTopo,
                  ptopo: DistributedWorld, nbsolpts: int, nb_elements_hori: int, nb_elements_vert: int,
                  case_number: int):
   ...
   df1_dx1, df2_dx2, df3_dx3, rhs = [jp.empty_like(Q, dtype=type_vec) for _ in range(4)]

   # Array for forcing: Coriolis terms, metric corrections from the curvilinear coordinate, and gravity
   forcing = jp.zeros_like(Q, dtype=type_vec)

   # Array to extrapolate variables and fluxes to the boundaries along x (i)
   variables_itf_i = jp.ones((nb_equations, nb_vertical_levels, nb_elements_hori + 2, 2, nb_pts_hori), dtype=type_vec) # Initialized to one in the halo to avoid division by zero later
   # Note that flux_x1_itf_i has a different shape than variables_itf_i
   flux_x1_itf_i   = jp.empty((nb_equations, nb_vertical_levels, nb_elements_hori + 2, nb_pts_hori, 2), dtype=type_vec)

   # Extrapolation arrays along y (j)
   variables_itf_j = jp.ones((nb_equations, nb_vertical_levels, nb_elements_hori + 2, 2, nb_pts_hori), dtype=type_vec) # Initialized to one in the halo to avoid division by zero later
   flux_x2_itf_j   = jp.empty((nb_equations, nb_vertical_levels, nb_elements_hori + 2, 2, nb_pts_hori), dtype=type_vec)

   # Extrapolation arrays along z (k), note dimensions of (6, nj, nk+2, 2, ni)
   variables_itf_k = jp.empty((nb_equations, nb_pts_hori, nb_elements_vert + 2, 2, nb_pts_hori), dtype=type_vec)
   flux_x3_itf_k   = jp.empty((nb_equations, nb_pts_hori, nb_elements_vert + 2, 2, nb_pts_hori), dtype=type_vec)

   # Special arrays for calculation of (ρw) flux
   wflux_adv_x1_itf_i = jp.zeros_like(flux_x1_itf_i[0,:])
   wflux_pres_x1_itf_i = jp.zeros_like(wflux_adv_x1_itf_i)
   wflux_adv_x2_itf_j = jp.zeros_like(flux_x2_itf_j[0,:])
   wflux_pres_x2_itf_j = jp.zeros_like(wflux_adv_x2_itf_j)
   wflux_adv_x3_itf_k = jp.zeros_like(flux_x3_itf_k[0,:])
   wflux_pres_x3_itf_k = jp.zeros_like(flux_x3_itf_k[0,:])
   ...
   variables_itf_i = variables_itf_i.at[:,:,1:-1,:,:].set(mtrx.extrapolate_i(Q,geom).transpose((0,1,3,4,2)))
   variables_itf_j = variables_itf_j.at[:,:,1:-1,:,:].set(mtrx.extrapolate_j(Q,geom))

   # Scaled variables for separate reconstruction
   logrho = jp.log(Q[idx_rho,:])
   logrhotheta = jp.log(Q[idx_rho_theta,:])

   variables_itf_i = variables_itf_i.at[idx_rho,:,1:-1,:,:].set(jp.exp(mtrx.extrapolate_i(logrho,geom)).transpose((0,2,3,1)))
   variables_itf_j = variables_itf_j.at[idx_rho,:,1:-1,:,:].set(jp.exp(mtrx.extrapolate_j(logrho,geom)))

   variables_itf_i = variables_itf_i.at[idx_rho_theta,:,1:-1,:,:].set(jp.exp(mtrx.extrapolate_i(logrhotheta,geom)).transpose((0,2,3,1)))
   variables_itf_j = variables_itf_j.at[idx_rho_theta,:,1:-1,:,:].set(jp.exp(mtrx.extrapolate_j(logrhotheta,geom)))
   ...
   pressure = p0 * jp.exp((cpd/cvd) * jp.log((Rd/p0)*Q[idx_rho_theta]))
   #pressure = Rd * Q[idx_rho_theta]

   wflux_pres_x1 = jp.zeros(metric.sqrtG.shape,dtype=type_vec)
   wflux_pres_x2 = jp.zeros(metric.sqrtG.shape,dtype=type_vec)
   wflux_pres_x3 = jp.zeros(metric.sqrtG.shape,dtype=type_vec)
