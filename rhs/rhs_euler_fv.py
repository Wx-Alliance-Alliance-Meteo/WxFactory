...
def rhs_euler_fv(
    ...
    for elem in range(nb_elements_hori):
        ...
        # --- Direction x1
        df1_dx1[:, :, :, epais] = flux_x1[:, :, :, epais]

        # --- Direction x2
        df2_dx2[:, :, epais, :] = flux_x2[:, :, epais, :]
    ...
    for slab in range(nb_pts_hori):
        for elem in range(nb_elements_vert):
            ...
            variables_itf_k[:, slab, pos, 0, :] = Q[:, epais, slab, :]
            variables_itf_k[:, slab, pos, 1, :] = Q[:, epais, slab, :]
    ...
    for slab in range(nb_pts_hori):
        for elem in range(nb_elements_vert):
            ...
            # print(f'mtrx.diff_solpt: {mtrx.diff_solpt}')
            # print(f'mtrx.correction: {mtrx.correction}')
            # raise ValueError
            # df3_dx3[:, epais, slab, :] = ( mtrx.correction @ flux_x3_itf_k[:, slab, elem+offset, :, :] ) #* 2.0 / geom.Δx3
            df3_dx3[:, epais, slab, :] = (
                flux_x3_itf_k[:, slab, elem + offset, 1, :] - flux_x3_itf_k[:, slab, elem + offset, 0, :]
            ) * 0.5
    ...
    for elem in range(nb_elements_hori):
        ...
        df1_dx1[:, :, :, epais] += (
            flux_x1_itf_i[:, :, elem + offset, :, 1] - flux_x1_itf_i[:, :, elem + offset, :, 0]
        ) * 0.5
        ...
        df2_dx2[:, :, epais, :] += (
            flux_x2_itf_j[:, :, elem + offset, 1, :] - flux_x2_itf_j[:, :, elem + offset, 0, :]
        ) * 0.5
    ...
    forcing[idx_rho_u1] = (
        2.0
        * (metric.christoffel_1_01 * rho * u1 + metric.christoffel_1_02 * rho * u2 + metric.christoffel_1_03 * rho * w)
    ...
    forcing[idx_rho_u2] = (
        2.0
        * (metric.christoffel_2_01 * rho * u1 + metric.christoffel_2_02 * rho * u2 + metric.christoffel_2_03 * rho * w)
    ...
    forcing[idx_rho_w] = (
        2.0
        * (metric.christoffel_3_01 * rho * u1 + metric.christoffel_3_02 * rho * u2 + metric.christoffel_3_03 * rho * w)
