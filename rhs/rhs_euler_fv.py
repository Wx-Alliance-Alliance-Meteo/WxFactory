...
#@profile
def rhs_euler_fv (Q: numpy.ndarray, geom: CubedSphere, mtrx: DFR_operators, metric: Metric_3d_topo, ptopo: Distributed_World, nbsolpts: int, nb_elements_hori: int, nb_elements_vert: int, case_number: int):
   ...
   for slab in range(nb_pts_hori):
      for elem in range(nb_elements_vert):
         ...
         df3_dx3[:, epais, slab, :] = ( flux_x3_itf_k[:, slab, elem+offset, 1, :] - flux_x3_itf_k[:, slab, elem+offset, 0, :]) * 0.5
   ...
   for elem in range(nb_elements_hori):
      ...
      df1_dx1[:, :, :, epais] += (flux_x1_itf_i[:, :, elem+offset, :, 1] - flux_x1_itf_i[:, :, elem+offset, :, 0]) * 0.5

      # --- Direction x2

      # df2_dx2[:, :, epais, :] += mtrx.correction @ flux_x2_itf_j[:, :, elem+offset, :, :]
      df2_dx2[:, :, epais, :] += (flux_x2_itf_j[:, :, elem+offset, 1, :] - flux_x2_itf_j[:, :, elem+offset, 0, :]) * 0.5
