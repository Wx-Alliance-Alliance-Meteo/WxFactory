...
def rhs_sw_stiff (Q: numpy.ndarray, geom, mtrx, metric, topo, ptopo, nbsolpts: int, nb_elements_hori: int):
   ...
   for itf in range(nb_interfaces_hori):
      ...
      flux_x1_itf_i[idx_h, elem_L, :, 1] = metric.sqrtG_itf_i[itf, :] * ( numpy.maximum(0., M) * aL * var_itf_i[idx_h, elem_L, 1, :] +  numpy.minimum(0., M) * aR * var_itf_i[idx_h, elem_R, 0, :] )
      ...
      flux_x2_itf_j[idx_h, elem_L, 1, :] = metric.sqrtG_itf_j[itf, :] * ( numpy.maximum(0., M) * aL * var_itf_j[idx_h, elem_L, 1, :] + numpy.minimum(0., M) * aR * var_itf_j[idx_h, elem_R, 0, :] )
   ...
   forcing[idx_hu1,:,:] = + metric.christoffel_1_11 * Q[idx_hu1] * u1 + 2.0 * metric.christoffel_1_12 * Q[idx_hu1] * u2 

   forcing[idx_hu2,:,:] = + 2.0 * metric.christoffel_2_12 * Q[idx_hu1] * u2 + metric.christoffel_2_22 * Q[idx_hu2] * u2 
