...
def rhs_sw_stiff(Q: numpy.ndarray, geom, mtrx, metric, topo, ptopo, nbsolpts: int, nb_elements_hori: int):
    ...
    # Initiate transfers
    all_request = ptopo.xchange_sw_interfaces(
        geom,
        var_itf_i[idx_h],
        var_itf_j[idx_h],
        var_itf_i[idx_hu1],
    ...
    hsquared = Q[idx_h] ** 2
    flux_x1[idx_hu1] = metric.sqrtG * (0.5 * gravity * metric.H_contra_11 * hsquared)
    flux_x2[idx_hu1] = metric.sqrtG * (0.5 * gravity * metric.H_contra_12 * hsquared)

    flux_x1[idx_hu2] = metric.sqrtG * (0.5 * gravity * metric.H_contra_21 * hsquared)
    flux_x2[idx_hu2] = metric.sqrtG * (0.5 * gravity * metric.H_contra_22 * hsquared)
    ...
    for itf in range(nb_interfaces_hori):
        ...
        p11_L = (
            ...
            * var_itf_i[idx_h, elem_L, 1, :] ** 2
        )
        p21_L = (
            metric.sqrtG_itf_i[itf, :]
            * 0.5
            * gravity
            * metric.H_contra_21_itf_i[itf, :]
            * var_itf_i[idx_h, elem_L, 1, :] ** 2
        )
        ...
        flux_x1_itf_i[idx_h, elem_L, :, 1] = metric.sqrtG_itf_i[itf, :] * (
            numpy.maximum(0.0, M) * aL * var_itf_i[idx_h, elem_L, 1, :]
            + numpy.minimum(0.0, M) * aR * var_itf_i[idx_h, elem_R, 0, :]
        )

        # --- Pressure part

        flux_x1_itf_i[idx_hu1, elem_L, :, 1] = 0.5 * ((1.0 + mL) * p11_L + (1.0 - mR) * p11_R)
        flux_x1_itf_i[idx_hu2, elem_L, :, 1] = 0.5 * ((1.0 + mL) * p21_L + (1.0 - mR) * p21_R)
        ...
        p12_L = (
            ...
            * 0.5
            * gravity
            * metric.H_contra_12_itf_j[itf, :]
            * var_itf_j[idx_h, elem_L, 1, :] ** 2
        )
        p22_L = (
            metric.sqrtG_itf_j[itf, :]
            * 0.5
        ...
        flux_x2_itf_j[idx_h, elem_L, 1, :] = metric.sqrtG_itf_j[itf, :] * (
            numpy.maximum(0.0, M) * aL * var_itf_j[idx_h, elem_L, 1, :]
            + numpy.minimum(0.0, M) * aR * var_itf_j[idx_h, elem_R, 0, :]
        )

        # --- Pressure part

        flux_x2_itf_j[idx_hu1, elem_L, 1, :] = 0.5 * ((1.0 + mL) * p12_L + (1.0 - mR) * p12_R)
        flux_x2_itf_j[idx_hu2, elem_L, 1, :] = 0.5 * ((1.0 + mL) * p22_L + (1.0 - mR) * p22_R)
