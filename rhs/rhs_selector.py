...
from geometry import Cartesian2D, CubedSphere, DFROperators, Geometry, Metric, Metric3DTopo
    ...
    def __init__(
        ...
        if param.equations == "euler" and isinstance(geom, CubedSphere):
            ...
            if param.case_number <= 1:  # Pure advection
                ...
                self.implicit = generate_rhs(
                    rhs_sw_stiff, geom, operators, metric, topo, ptopo, param.nbsolpts, param.nb_elements_horizontal
                )
                self.explicit = generate_rhs(
                    rhs_sw_nonstiff, geom, operators, metric, topo, ptopo, param.nbsolpts, param.nb_elements_horizontal
                )
