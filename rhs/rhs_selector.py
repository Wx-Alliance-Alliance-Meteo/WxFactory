      ...
      if param.equations == "euler" and isinstance(geom, CubedSphere):
         if param.discretization == 'dg':
            self.full = lambda q: rhs_euler(
               q.reshape(self.shape), geom, operators, metric, ptopo, param.nbsolpts, param.nb_elements_horizontal,
         ...
         if param.discretization == 'dg':
            self.full = lambda q: rhs_bubble(
               q.reshape(self.shape), geom, operators, param.nbsolpts, param.nb_elements_horizontal, param.nb_elements_vertical)
         elif param.discretization == 'fv':
         ...
         if param.case_number <= 1: # Pure advection
            ...
            self.full = lambda q: rhs_sw(
               q.reshape(self.shape), geom, operators, metric, topo, ptopo, param.nbsolpts, param.nb_elements_horizontal)
