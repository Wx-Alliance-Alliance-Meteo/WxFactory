...
def rhs_bubble_implicit(Q, geom, mtrx, nbsolpts, nb_elements_x, nb_elements_z):
    ...
    for itf in range(1, nb_interfaces_z - 1):
        ...
        kfaces_flux[idx_2d_rho, right, 0, :] = (kfaces_var[idx_2d_rho, left, 1, :] * numpy.maximum(0.0, M) * a_L) + (
            kfaces_var[idx_2d_rho, right, 0, :] * numpy.minimum(0.0, M) * a_R
        )
        kfaces_flux[idx_2d_rho_w, right, 0, :] += 0.5 * (
            (1.0 + M_L) * kfaces_pres[left, 1, :] + (1.0 - M_R) * kfaces_pres[right, 0, :]
        )
    ...
    for itf in range(1, nb_interfaces_x - 1):
        ...
        ifaces_flux[idx_2d_rho, right, :, 0] = (ifaces_var[idx_2d_rho, left, :, 1] * numpy.maximum(0.0, M) * a_L) + (
            ifaces_var[idx_2d_rho, right, :, 0] * numpy.minimum(0.0, M) * a_R
        )
        ifaces_flux[idx_2d_rho_u, right, :, 0] += 0.5 * (
            (1.0 + M_L) * ifaces_pres[left, :, 1] + (1.0 - M_R) * ifaces_pres[right, :, 0]
        )
    ...
    for elem in range(nb_elements_z):
        ...
        df3_dx3[idx_2d_rho, epais, :] = (
            (mtrx.diff_solpt @ flux_x3[idx_2d_rho, epais, :] + mtrx.correction @ kfaces_flux[idx_2d_rho, elem, :, :])
            * 2.0
            / geom.Δx3
        )
        df3_dx3[idx_2d_rho_w, epais, :] = (
            (
                mtrx.diff_solpt @ flux_x3[idx_2d_rho_w, epais, :]
                + mtrx.correction @ kfaces_flux[idx_2d_rho_w, elem, :, :]
            )
            * 2.0
            / geom.Δx3
        )
    ...
    for elem in range(nb_elements_x):
        ...
        df1_dx1[:-2, :, epais] = (
            (flux_x1[:-2, :, epais] @ mtrx.diff_solpt.T + ifaces_flux[:-2, elem, :, :] @ mtrx.correction.T)
            * 2.0
            / geom.Δx1
        )
