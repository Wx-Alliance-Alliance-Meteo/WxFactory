...
def rhs_bubble(Q, geom, mtrx, nbsolpts, nb_elements_x, nb_elements_z):
    ...
    for itf in range(1, nb_interfaces_z - 1):
        ...
        kfaces_flux[:, right, 0, :] = (kfaces_var[:, left, 1, :] * numpy.maximum(0.0, M) * a_L) + (
            kfaces_var[:, right, 0, :] * numpy.minimum(0.0, M) * a_R
        )
    ...
    for itf in range(start, nb_interfaces_x - 1):
        ...
        ifaces_flux[:, right, :, 0] = (ifaces_var[:, left, :, 1] * numpy.maximum(0.0, M) * a_L) + (
            ifaces_var[:, right, :, 0] * numpy.minimum(0.0, M) * a_R
        )
    ...
    for elem in range(nb_elements_z):
        ...
        df3_dx3[:, epais, :] = (
            mtrx.diff_solpt @ flux_x3[:, epais, :] + mtrx.correction @ kfaces_flux[:, elem, :, :]
        ) * factor
