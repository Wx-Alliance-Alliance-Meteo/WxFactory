...
def rhs_bubble(Q, geom, mtrx, nbsolpts, nb_elements_x, nb_elements_z):
   ...
   # ------ Vertical
   a_T = sound_itf[nb_elements_x:, bot_itf]
   a_B = sound_itf[:-nb_elements_x, top_itf]

   M_T = mach_itf[nb_elements_x:, bot_itf]
   M_B = mach_itf[:-nb_elements_x, top_itf]

   M = 0.25 * ((M_B + 1.)**2 - (M_T - 1.)**2)

   common_flux[:, nb_elements_x:, bot_itf] = (var_itf[:, :-nb_elements_x, top_itf] * numpy.maximum(0., M) * a_B) + \
                                             (var_itf[:, nb_elements_x:, bot_itf] * numpy.minimum(0., M) * a_T)

   common_flux[idx_2d_rho_w, nb_elements_x:, bot_itf] = 0.5 * ((1. + M_B) * pressure_itf[:-nb_elements_x, top_itf] + \
                                                               (1. - M_T) * pressure_itf[nb_elements_x:, bot_itf]).T

   common_flux[:, :-nb_elements_x, top_itf] = common_flux[:, nb_elements_x:, bot_itf]

   # Zero flux BC at bottom and top boundaries
   # except for (vertical) momentum, where we use extrapolated pressure at boundary
   common_flux[:,  :nb_elements_x, bot_itf] = 0.0
   common_flux[:, -nb_elements_x:, top_itf] = 0.0
   common_flux[idx_2d_rho_w,  :nb_elements_x, bot_itf] = pressure_itf[ :nb_elements_x, bot_itf].T
   common_flux[idx_2d_rho_w, -nb_elements_x:, top_itf] = pressure_itf[-nb_elements_x:, top_itf].T


   # ------ Horizontal
   # Need to ajust Mach number for horizontal direction
   mach_itf = var_itf[idx_2d_rho_u] / (var_itf[idx_2d_rho] * sound_itf)

   a_W = sound_itf[:-1, east_itf]
   a_E = sound_itf[1:, west_itf]

   M_W = mach_itf[:-1, east_itf]
   M_E = mach_itf[1:, west_itf]

   M = 0.25 * ((M_W + 1.)**2 - (M_E - 1.)**2)

   common_flux[:, 1:, west_itf] = (var_itf[:, :-1, east_itf] * numpy.maximum(0., M) * a_W) + \
                                  (var_itf[:, 1:, west_itf] * numpy.minimum(0., M) * a_E)

   common_flux[idx_2d_rho_u, 1:, west_itf] = 0.5 * ((1. + M_W) * pressure_itf[:-1, east_itf] + \
                                                    (1. - M_E) * pressure_itf[1:, west_itf]).T

   common_flux[:, :-1, east_itf] = common_flux[:, 1:, west_itf]

   # Boundary conditions
   n0 = nb_elements_x - 1
   jump = nb_elements_x
   common_flux[:,   ::jump, west_itf] = 0.0
   common_flux[:, n0::jump, east_itf] = 0.0
   common_flux[idx_2d_rho_u,   ::jump, west_itf] = pressure_itf[  ::jump, west_itf].T
   common_flux[idx_2d_rho_u, n0::jump, east_itf] = pressure_itf[n0::jump, east_itf].T
