...
def rhs_bubble(Q, geom, mtrx, nbsolpts, nb_elements_x, nb_elements_z, device: Device = default_device):
   xp = device.xp
   ...
   common_flux_z[:, nb_elements_x:, bot_itf] = (var_itf_z[:, :-nb_elements_x, top_itf] * xp.maximum(0., M) * a_B) + \
                                             (var_itf_z[:, nb_elements_x:, bot_itf] * xp.minimum(0., M) * a_T)
   ...
   common_flux_x[:, 1:, west_itf] = (var_itf_x[:, :-1, east_itf] * xp.maximum(0., M) * a_W) + \
                                  (var_itf_x[:, 1:, west_itf] * xp.minimum(0., M) * a_E)
