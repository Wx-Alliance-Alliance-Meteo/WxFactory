...
def rhs_sw(Q: numpy.ndarray, geom, mtrx, metric, topo, ptopo, nbsolpts: int, nb_elements_hori: int):
    ...
    flux_x2_itf_j, var_itf_i, var_itf_j = [
        numpy.empty((nb_equations, nb_elements_hori + 2, 2, nbsolpts * nb_elements_hori), dtype=type_vec)
        for _ in range(3)
    ]
    ...
    for itf in range(nb_interfaces_hori):
        ...
        flux_x1_itf_i[:, elem_L, :, 1] = metric.sqrtG_itf_i[itf, :] * (
            numpy.maximum(0.0, M) * aL * var_itf_i[:, elem_L, 1, :]
            + numpy.minimum(0.0, M) * aR * var_itf_i[:, elem_R, 0, :]
        )
        ...
        flux_x2_itf_j[:, elem_L, 1, :] = metric.sqrtG_itf_j[itf, :] * (
            numpy.maximum(0.0, M) * aL * var_itf_j[:, elem_L, 1, :]
            + numpy.minimum(0.0, M) * aR * var_itf_j[:, elem_R, 0, :]
        )
