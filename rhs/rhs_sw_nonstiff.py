...
def rhs_sw_nonstiff (Q: numpy.ndarray, geom, mtrx, metric, topo, ptopo, nbsolpts: int, nb_elements_hori: int):
   ...
   for itf in range(nb_interfaces_hori):
      ...
      flux_x1_itf_i[1:, elem_L, :, 1] = metric.sqrtG_itf_i[itf, :] * ( numpy.maximum(0., M) * aL * var_itf_i[1:, elem_L, 1, :] +  numpy.minimum(0., M) * aR * var_itf_i[1:, elem_R, 0, :] )
      ...
      flux_x2_itf_j[1:, elem_L, 1, :] = metric.sqrtG_itf_j[itf, :] * ( numpy.maximum(0., M) * aL * var_itf_j[1:, elem_L, 1, :] + numpy.minimum(0., M) * aR * var_itf_j[1:, elem_R, 0, :] )
