...
class ExponentialSmoother(Smoother):
    def __init__(
        self, niter: int = 4, target_spectral_radius: float = 1.0, global_dt: float = 1.0, verbose: bool = False
    ) -> None:
        super().__init__()
        self.niter = niter
    ...
    def __smoothe__(self, A: _MatvecOp, b: numpy.ndarray, x0: numpy.ndarray) -> numpy.ndarray:
        ...
        if self.verbose:
            print("x0 residual", x0_residual_norm)
        ...
        while j < self.niter:
            ...
            if curr_nrm < happy_tol:
                happy = True
                if self.verbose:
                    print(" ---> Happy breakdown in exponential smoother")
                break
        ...
        if self.verbose:
            print("Spectral radius of H:", max(abs(D_eig)))
        if self.verbose:
            print("dt:", dt)
        ...
        if new_residual_norm > x0_residual_norm:
            if self.verbose:
                print(
                    f"Smoother failed to reduce the norm of the residual. "
