...
class MultigridLevel:
    ...
    def __init__(
        ...
        if verbose > 0:
            ...
            if p.mg_smoother == "exp":
                out_str += f" exp radius: {p.exp_smoothe_spectral_radius}"
            print(out_str)

        # Initialize problem for this level
        if p.grid_type == "cubed_sphere":
            self.geometry = CubedSphere3D(
                p.num_elements_horizontal, p.num_elements_vertical, p.num_solpts, p.lambda0, p.phi0, p.alpha0, p.ztop, ptopo, p
            )
    ...
    def prepare(
        self, dt: float, field: numpy.ndarray, prev_field: Optional[numpy.ndarray] = None
        ...
        if self.param.mg_smoother in ["erk1", "erk3", "ark3"]:
            ...
            if isinstance(self.geometry, Cartesian2D):
                delta_min = abs(1.0 - self.geometry.solutionPoints[-1]) * min_geo
                ...
                delta_min = abs(1.0 - self.geometry.solutionPoints[-1])
        ...
        if self.param.time_integrator in ["ros2", "rosexp2", "partrosexp2", "strang_epi2_ros2", "strang_ros2_epi2"]:
            self.matrix_operator = functools.partial(
                matvec_rat, dt=dt, Q=field, rhs=self.rhs.full(field), rhs_handle=self.rhs.full
            )
...
class Multigrid(MatvecOp):
    ...
    def __init__(
        self, param: Configuration, ptopo: ProcessTopology, device: Device, discretization: str, fv_only: bool = False
    ) -> None:
        ...
        if self.verbose:
            print(
                f"orders: {self.orders}, h elem counts: {self.elem_counts_hori}, v elem counts: {self.elem_counts_vert}"
            )
        ...
        if discretization == "fv":
            ...
            if not fv_only:
                ...
                for d in dims:
                    self.big_shape[d] = self.big_shape[d] * int_shape[1] // int_shape[0]
            ...
            self.get_solution_back = lambda vec, op=self.initial_interpolator, sh=self.levels[0].shape: op(
                vec.reshape(sh), reverse=True
            )
    ...
    def iterate(
        ...
        if level < num_levels - 1:
            ...
            before_res = 0.0
            if verbose:
                before_res = global_norm(b - A(x).flatten())
            t0 = time()
            x, _, _, num_iter, _, _ = fgmres(
                A, b, x0=x, tol=lvl_param.param.precond_tolerance, restart=100, verbose=False
            )
            t1 = time()
            if verbose:
                corr_res, rel = self.compare_res(A, b, x, before_res)
    ...
    ####################
    def solve(
        self, b, x0=None, field=None, dt=None, coarsest_level=1, tolerance=1e-7, gamma=1, max_num_it=100, verbose=False
    ):
        """
