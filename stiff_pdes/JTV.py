...
def nonLinLapPeriodic(vec, u, epsilon, world):
    ...
    if world.size > 1:
        ...
        [TopNeighData, BottomNeighData, LeftNeighData, RightNeighData] = getExtraPeriodicData(
            vec, world, topData, bottomData, leftData, rightData
        )

        [JacUTopData, JacUBotData, JacULeftData, JacURightData] = getNeighData(
            u, world, jactopData, jacbottomData, jacleftData, jacrightData
        )

        [JacUTopData, JacUBotData, JacULeftData, JacURightData] = getExtraPeriodicData(
            u, world, jactopData, jacbottomData, jacleftData, jacrightData
        )
        ...
        for i in range(0, world.numPointsX):  # length of data
            ...
            if j == 0:
                ...
                # if not, use data from neighbooring processor
                    ...
                    current_val += (
                        vec[indx + world.numPointsX] * u[indx + world.numPointsX] - BottomNeighData[i] * JacUBotData[i]
                    )
                ...
                # else, use data from neighboring processors
                    ...
                    current_val += (
                        TopNeighData[i] * JacUTopData[i] - vec[indx - world.numPointsX] * u[indx - world.numPointsX]
                    )

            else:  # have acces to data within processors
                # print("j = {}, i = {}, indx = {}".format(j, i, indx))
                current_val += (
                    vec[indx + world.numPointsX] * u[indx + world.numPointsX]
                    - vec[indx - world.numPointsX] * u[indx - world.numPointsX]
                )
