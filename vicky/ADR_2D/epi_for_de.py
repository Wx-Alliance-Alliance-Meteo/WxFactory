...
class Epi_for_DE():
   def __init__(self,  order: int, rhs: Callable, tol:float, exponential_solver: str, exode_method: str, exode_controller: str, init_method=None, init_substeps: int = 60):
      #super().__init__(tol,exponential_solver, exode_method, exode_controller, preconditioner=None)
      self.rhs = rhs
      self.tol = tol
      self.krylov_size = 1
      #self.jacobian_method = param.jacobian_method
      self.exponential_solver = exponential_solver
      self.exode_method = exode_method
      self.exode_controller = exode_controller
      ...
      if init_method or self.n_prev == 0:
         ...
         self.init_method = Epi_for_DE(2, rhs, tol, exponential_solver, exode_method, exode_controller)

      self.init_substeps = init_substeps

   def step(self, Q: numpy.ndarray, dt: float, first_step: float):
      ...
      if len(self.previous_Q) < self.n_prev:
         ...
         for i in range(self.init_substeps):
            #print("substep=", i)
            Q,stats = self.init_method.step(Q, dt, first_step)
         return Q,stats
      ...
      if self.exponential_solver == 'exode':
         phiv, stats = exode(first_step, matvec_handle, vec, method=self.exode_method, controller=self.exode_controller, rtol = self.tol, atol = self.tol, task1 = False, verbose=False)
...
#         self.solver_info = SolverInfo(total_num_it = stats[2])
      ...
      # Update solution
      return Q + numpy.reshape(phiv, Q.shape) * dt, stats
