...
class EpiStiff_for_DE():
   def __init__(self, order: int, rhs: Callable, tol:float, exponential_solver: str, exode_method: str, exode_controller: str, init_method=None, init_substeps: int = 1):
   ...
   def step(self, Q: numpy.ndarray, dt: float):
      ...
      if self.exponential_solver == 'exode':
         ...
         phiv, stats = exode(1., matvec_handle, vec, method=self.exode_method, rtol = self.tol, atol = self.tol, task1 = False, verbose=False)
